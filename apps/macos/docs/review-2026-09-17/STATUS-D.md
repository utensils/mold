...
| --- | --- | --- | --- |
| Queue row | Try Again, Move Up/Down, Cancel Job — wrong order vs the Queue menu, no Pause/Resume, no divider | Pause Job, Resume Job, Try Again, Move Up, Move Down, divider, **Cancel Job** | `QueueRowActions` (+ `menuTitles`) |
| Queue batch row | Move Up/Down only | Pause Every Job, Resume Every Job, Move Up, Move Down, divider, **Cancel Every Job** | `QueueRowActions.group` + `QueueBatchRow.menuTitles` |
| Queue held row | already correct (`d9421ae0`) | unchanged | `QueueHoldRow.menuTitles` |
...
| Machines ▸ machine row | Show in Library, Check Now | Check Now, Set as Default, divider, Show in Library | mirrors `MachineCommands.swift:21-25` |
...
a pure title list instead (`menuTitles`, `menuItems`, `menuItem(named:)`).
...
- `QueueStoreLiveTests.aJobFrameReReadsThatMachineOnceForABurst` and its
  ...
  themselves, and the read count is asserted as `1...2`: per the coordinator,
  a stream that dropped a frame is allowed exactly one more re-read (its
  `.resyncRequired`, which skips the coalescing delay by design). 64 frames
  are still never 64 reads, which is the whole contract.
...
- `settle(until:)`'s budget went from ~0.5 s to ~2 s. The whole app bundle
  shares ONE main actor and a suite running beside three others can be starved
  of it for longer than half a second. It still returns the instant the
...
Full `xcodebuild … test` on the app bundle — 411 tests in 58 suites, green.
