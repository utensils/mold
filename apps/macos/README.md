...
### The one-time owner step

The EdDSA signing key is not in this repository and cannot be generated by
anyone but the owner. Until it is done, a Release **fails to build** rather
than shipping an updater that can never verify anything; Debug is unaffected.
...
`apps/macos/project.yml` as `MOLD_SPARKLE_PUBLIC_ED_KEY`, replacing
`REPLACE_WITH_SPARKLE_PUBLIC_KEY`. A public key is not a secret; it ships
...
see "Updates". What it still needs is the owner's one-time key step, and a
first run of the publish workflow.)
