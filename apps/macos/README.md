...
The engine starts **at launch** unless Settings ▸ This Mac ▸ "Start the engine
when Mold opens" is off (`engineStartsAtLaunch`; absent means on, and a test
run never starts one -- the tests use the app as their host). It starts **at
most once per process** (mold's models-dir override is a process-lifetime
`OnceLock`), which is why Settings ▸ This Mac offers Relaunch Mold rather than
an inert Start after a failure that has spent it -- the same whether the launch
or the button spent it. Whoever started it, the engine answering is what puts
This Mac in the machine list (`MoldEngine.adoptsItsMachine`).

`make build` relinks whatever staticlib `make engine` last produced, so a
Debug build can quietly carry an engine far behind the branch. `make engine`
stamps the commit it built from beside the library, and `make build` warns
(never fails) when `crates/`, `apps/macos/rust/` or `Cargo.lock` changed since
-- run `make engine` then.
