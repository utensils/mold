...
struct DownloadsPopoverTests {
    ...
    @Test func aCancelledRowLeavesTheListAndAFailedOneStaysWithItsReason() async {
        ...
        await downloads.cancel(jobID: "job-cancel", on: workstation)
        downloads.apply(
        ...
        let rows = DownloadsPopover.Rows.resolve(
            active: downloads.active[workstation.id] ?? [:], finished: downloads.finished[workstation.id] ?? [])
    ...
    @Test func clearEmptiesOnlyThatMachinesFinishedRows() {
        ...
        let fake = FakeBackend(host: workstation)
        let hosts = HostStore(hosts: [workstation, hal]) { _ in fake }
        let downloads = DownloadStore(hosts: hosts, licenses: LicenseStore(hosts: hosts))
        downloads.apply(
            FakeFixtures.downloadEvent(type: "job_done", id: "job-workstation", model: "flux-dev:q4"), on: workstation.id)
        downloads.apply(
            FakeFixtures.downloadEvent(type: "job_done", id: "job-hal", model: "sd15:fp16"), on: hal.id)

        downloads.clearFinished(on: workstation.id)

        #expect(DownloadsPopover.Rows.resolve(active: [:], finished: downloads.finished[workstation.id] ?? []).isEmpty)
        #expect(
