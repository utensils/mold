...
struct HostStoreLifecycleTests {
    ...
    @Test func checkingAMachineThatIsAlreadyUpKeepsItsWatcher() async {
        ...
        hosts.reachability[plato.id] = .checking
    ...
    @Test func forgettingAMachineClearsEverythingItLeftBehind() async {
        ...
        hosts.report(MoldClientError.unreachable("boom"), on: plato.id, doing: "list its prints")

        hosts.remove(plato)

        #expect(hosts.reachability[plato.id] == nil)
        #expect(hosts.capabilities[plato.id] == nil)
        #expect(hosts.exportOptions[plato.id] == nil)
        #expect(hosts.instanceIDs[plato.id] == nil)
        #expect(hosts.watchers[plato.id] == nil)
    ...
    @Test func changingAMachinesAddressForgetsTheFleetIdentityItHad() async {
        ...
        hosts.update(MoldHost(id: plato.id, name: "plato",
                              baseURL: URL(string: "http://plato-2:7680")!))

        #expect(hosts.instanceIDs[plato.id] == nil)
        #expect(hosts.instanceID(of: plato.id) == nil)
    }
    ...
    @Test func theInstanceIdComesFromTheAuthorityFrameThenTheStatus() async {
        let plato = machine()
        let backend = fake(for: plato, instanceId: "from-status")
        let hosts = HostStore(hosts: [plato]) { _ in backend }

        await hosts.refresh(plato)
        #expect(hosts.instanceID(of: plato.id) == "from-status")
        ...
        #expect(hosts.instanceID(of: plato.id) == "from-the-stream")
    }
}
