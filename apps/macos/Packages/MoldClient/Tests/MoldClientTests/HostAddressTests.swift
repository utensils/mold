...
@Test(arguments: [
    ...
    ("::1", "http://[::1]:7680"),
    ("workstation", "http://workstation:7680"),
    ("  workstation  ", "http://workstation:7680"),
    ("http://workstation:7680/", "http://workstation:7680"),
    ("HTTP://Workstation:7680", "http://workstation:7680"),
])
...
@Test(arguments: [
    ...
    "https://box.tail1234.ts.net", "https://box.tail1234.ts.net:443", "::1", "workstation",
])
...
@Test func refusesInputThatIsNotAnAddressAndSaysWhy() {
    ...
    #expect(throws: HostAddress.Problem.unsupportedScheme) { try HostAddress.resolve("ftp://workstation") }
}
...
@Test func hidesTheOrdinarySchemeWhenShowingAnAddress() throws {
    let plain = try #require(HostAddress.normalize("workstation"))
    #expect(HostAddress.displayString(for: plain) == "workstation:7680")
...
@Test func recognizesTwoSpellingsOfOneMachine() throws {
    let typed = try #require(HostAddress.normalize("WORKSTATION"))
    let saved = try #require(HostAddress.normalize("http://workstation:7680/"))
    #expect(HostAddress.sameOrigin(typed, saved))
    let other = try #require(HostAddress.normalize("workstation:7681"))
