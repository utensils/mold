...
#[cfg(test)]
mod tests {
    use super::{flux_runtime_dtype, flux_transformer_var_builder, park_cond_to_cpu};
    ...
    /// When the input lives somewhere else (GPU in production, CPU here in
    /// the test runner), `park_cond_to_cpu` must yield a CPU tensor with the
    /// same shape and dtype. The CPU-only test runner can't model the GPU
    /// case directly, but the contract is "output device is CPU regardless
    /// of input device" — that's what we pin.
    #[test]
    fn park_cond_to_cpu_returns_cpu_tensor_for_any_input() {
        ...
        assert!(
            parked.device().is_cpu(),
            "park_cond_to_cpu output must always be on CPU"
        );
