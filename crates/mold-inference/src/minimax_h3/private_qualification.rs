...
mod tests {
    ...
    /// resolve to the base manifest OF THE TAG'S OWN TASK. Passing the tag
    /// itself is the exact live failure "requires an exact Comfy H3 canonical
    /// model name".
    #[test]
    fn turbo_tags_qualify_through_their_engine_partition() {
        for tier in contract::REVIEWED_TURBO_MANIFEST_TIERS {
            ...
            assert_eq!(
                manifest.name,
                contract::base_compact_model_for_task(expected_task)
            );
            assert_eq!(
                published,
                match expected_task {
                    Task::Fl2va => H3ComfyPublishedArtifact::Fl2VaPrunedInt8ConvRot,
                    Task::Ref2va => H3ComfyPublishedArtifact::Ref2VaPrunedInt8ConvRot,
                }
            );
        }
    }
    ...
    fn cheap_precheck_accepts_writable_artifacts_and_rejects_symlinks() {
        ...
        fs::write(&target, b"aliased").unwrap();
        let link = directory.path().join("link.safetensors");
        symlink(&target, &link).unwrap();
        let aliased = ResolvedArtifact {
            file: &manifest.files[0],
