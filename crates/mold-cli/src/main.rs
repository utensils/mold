...
Examples:
  ...
  echo \"a dog\" | mold run flux-schnell")]
    Run {
        ...
        /// latent carryover (simple concat). Default 25 — four LTX-2 latent
        /// frames of carryover at the 8× causal temporal compression (causal-
        /// first slot + three continuation slots, ≈1.04 s at 24 fps), enough
        /// hard-pinned pixel context to keep scene identity and motion
        /// coherent across clips.
        #[arg(long, value_name = "N", default_value_t = 25, help_heading = "Video")]
        motion_tail: u32,
...
mod tests {
    ...
    #[test]
    fn run_motion_tail_defaults_to_twenty_five() {
        ...
        match cli.command {
            Commands::Run {
                ...
                assert_eq!(
                    motion_tail, 25,
                    "default motion tail must be 25 frames (four LTX-2 latent frames: causal + three continuation)"
                );
