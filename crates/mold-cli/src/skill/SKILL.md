...
- `POST /api/chain-jobs` — chained arbitrary-length video (LTX-2, LTX-Video, Wan); body is `mold_core::chain::ChainRequest` (canonical `stages[]` or auto-expand `prompt`+`total_frames`+`clip_frames`); returns a job id, and `GET /api/chain-jobs/{id}/events` streams stage progress to settlement. The synchronous and SSE `POST /api/generate/chain[/stream]` compatibility endpoints are gone: they ran a chain as a hidden ephemeral job that could not be resumed, retaken, or reattached
