...
- `POST /api/generation-batches` — durable idempotent Batch N admission (`batch_size: 1` per child); recover with `GET /api/generation-batches/by-client/:client_batch_id`, poll with `GET /api/generation-batches/:id`, and reconcile sets with `POST /api/generation-batches/status`
