...
- `POST /api/chain-jobs` — chained arbitrary-length video (LTX-2, LTX-Video, Wan); additive `ephemeral: true` marks an auto-chained one-shot (absent from the listing, no `chain_job_queued`, swept, resume refused, print carries no `chain_job_id` so Reuse settings restores a one-shot); requires `output_format: mp4`; body is `mold_core::chain::ChainRequest` (canonical `stages[]` or auto-expand `prompt`+`total_frames`+`clip_frames`); returns a job id, and `GET /api/chain-jobs/{id}/events` streams stage progress to settlement. The synchronous and SSE `POST /api/generate/chain[/stream]` compatibility endpoints are gone: they ran a chain as a hidden ephemeral job that could not be resumed, retaken, or reattached
