...
- `POST /api/generation-batches` — durable idempotent 1..N admission (`batch_size: 1` per child) on protocol-v2 hosts; recover with `GET /api/generation-batches/by-client/:client_batch_id`, poll with `GET /api/generation-batches/:id`, and reconcile sets with `POST /api/generation-batches/status`
