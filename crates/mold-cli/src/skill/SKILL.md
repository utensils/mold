...
- If arguments start with a **subcommand** (`pull`, `list`, `default`, `config`, `serve`, `server`, `mcp`, `info`, `ps`, `rm`, `unload`, `update`, `stats`, `clean`, `tui`, `completions`, `version`, `runpod`, `lambda`, `jobs`, `queue`, `library`, `chain`, `gpu`, `trash`, `licenses`, `expand`, `remix`, `upscale`, `discord`, `skill`), run that subcommand
...
`mold remix <SOURCE-PROMPT>` is the sibling command: it returns
subject-preserving alternatives of an existing prompt rather than a longer
version of a short one, over `POST /api/remix` on the same expander (advertised
as `capabilities.expand.remix`).

```bash
# Three alternatives of an existing prompt
mold remix "a red apple on a wooden table"

# Family-aware policy, more alternatives, structured output
mold remix "a cat" --model ltx-2-19b-distilled:fp8 --variations 5 --json

# Vary named creative dimensions while locking a style
mold remix "a cat" --dimensions lighting,composition --style "35mm film"
```

Flags: `--model`, `--variations` (default 3), `--json`, `--backend`,
`--expand-model`, `--task <TASK>`, `--source original|current|direct`
(default `direct`), `--root-prompt`, `--dimensions` (repeatable or
...
- Server-owned batch planning reads one static `mold-inference` family registry before engine load, constructs every advertised alias in tests, and validates the instantiated runtime against it. Every production family currently declares only native size `[1]`; initial-wave cardinality is lexically first, homogeneous singleton planning is exact at arbitrary size, and host-coupled heterogeneous singleton planning is exact through 8 devices at arbitrary child count. Singleton results retain one arithmetic lane per selected device with typed random access and bounded 4096-record windows. Ordinary native inputs expose the deterministic 64-strategy bounded heuristic; huge batches use the mandatory compact singleton fallback as `BoundedHeuristic`. A remote `--batch N` above 1 is admitted as ordered singleton children through `/api/generation-batches`, reconciled by client/batch ID, and chunked above `queue.heterogeneous_batch_max_outputs`. A single-image remote generation stays on `/api/generate[/stream]`, which is the same durable admission with an attached observer and is the path that carries live step progress, the denoise preview, and missing-model auto-pull. There is no non-durable fallback: a host that cannot admit durably refuses every generation route with `503 DURABLE_ADMISSION_UNAVAILABLE`. No engine is represented as returning multiple native outputs.
...
Generate video clips with the LTX Video and Wan families. Every video family (LTX Video, LTX-2, Wan, MiniMax H3) defaults to MP4 in release builds; a build without the `mp4` feature falls back to APNG (lossless, with metadata). Canonical: `website/guide/generating.md`.

```bash
# Basic video generation (25 frames, MP4 output)
...
**Output formats:** `mp4` (H.264, the default for every video family in a build carrying the `mp4` feature — every release recipe except Windows), `apng` (lossless, metadata; the fallback default without `mp4`), `gif` (256 colors), `webp` (requires `webp` feature), `wav` (16-bit PCM, LTX-2 `--pipeline t2a` only).
...
**Models:** `ltx-2-19b-dev:fp8`, `ltx-2-19b-distilled:fp8`, `ltx-2.3-22b-dev:fp8`, `ltx-2.3-22b-distilled:fp8`, `ltx-2.5-22b-dev:{int8-conv,bf16-conv,bf16}`, `ltx-2.5-22b-distilled:{int8-conv,bf16-conv,bf16,q3-k-s,q3,q4-k-s,q4,q5,q6,q8}` (the `q*` tags are the pinned Abiray GGUF transformer tiers, QTensor-at-rest with the official companion pack)
...
**Important flags:** `--audio`, `--no-audio`, `--video-only` (skip the audio branch entirely; output-changing, conflicts with `--audio`/`--audio-file`), `--audio-file`, `--video`, repeatable `--keyframe`, repeatable `--lora`, `--pipeline`, `--retake`, `--camera-control`, `--spatial-upscale`, `--temporal-upscale`, `--clip-frames`, `--motion-tail`, `--stg-scale`, `--stg-blocks`, `--rescale-scale`, `--modality-scale`, `--guidance-skip-step`, `--hdr-exr-dir`, `--hdr-exr-full-float`
...
**LTX-2's frame ceiling is a duration, not a frame count.** The checkpoints ship `pos_embed_max_pos = 20` and the temporal RoPE axis is normalized in _seconds_ (the pixel-frame coordinate is divided by fps before `max_pos` normalization), so the real budget is 20 s of runtime: `frames <= 20 * fps + 4`, capped by a 604-frame resource guard. The advertised value is snapped down onto the `8n+1` grid the validator enforces (`ltx2_max_frames_on_grid_at_fps`), so it is 481 frames at 24 fps, 241 at 12 fps, and only 121 at 6 fps — the ceiling moves in both directions, so never hard-code it. `--temporal-upscale x2` does not extend it: `derive_stage1_render_shape` halves the stage-1 frame count _and_ the stage-1 fps, so stage 1 renders the same runtime at half the frame rate.
...
On Apple Metal, LTX-2.5 GGUF uses the same packed-block adaptive shape against a live unified-memory sample after reserving `max(15% of installed RAM, 8 GiB)` for macOS and other applications. Blocks outside that budget remain on disk: the GGUF backend seeks and reads one tensor at a time, and the transformer fences each bounded streaming window so Metal command buffers release those temporary weights before the next window can accumulate. If pressure changes during resident-block construction, the loader demotes blocks until the split fits. When the fixed weights, request activations, runtime headroom, and one streamed block cannot fit without crossing the live floor, the request fails before transformer allocation. A failed release fence after allocation OOM also fails closed instead of retrying into unresolved pressure. CUDA's established residency and fence cadence are unchanged.

Q3_K_M (`:q3`), Q4_K_M (`:q4`), and Q6_K (`:q6`) are hardware-qualified on a 48 GiB Apple M4 Max at 512x512, 9 frames, and 8 steps with fixed-seed visual prompt-fidelity inspection; Q3_K_M also completed a 97-frame MP4. The `:q3-k-s`, `:q4-k-s`, `:q5`, and `:q8` tiers remain runnable but are not claimed by that Metal qualification campaign.
...
- Chain endpoint: `POST /api/chain-jobs` (durable), followed by `GET /api/chain-jobs/{id}/events` for stage progress
...
`mold library` browses and organizes one serving host's live gallery. Every
non-`grid` action talks HTTP to `MOLD_HOST` (with `MOLD_API_KEY` when set) and
never falls back to direct filesystem access; `grid` delegates to the TUI
Library so terminal protocol detection stays singular.

```bash
mold library list [--query TEXT] [--tag TAG]... [--collection NAME-OR-SLUG] [--favorite] \
                  [--format png|jpeg|gif|apng|webp|mp4|wav] [--limit 50] [--offset 0] [--json]
mold library show <FILENAME> [--json | --preview]   # metadata, or inline terminal preview
mold library grid [--host URL | --local]            # protocol-aware terminal Library grid
mold library title <FILENAME> "New title"           # PATCH the title; --clear removes it
mold library favorite <FILENAME>...                 # and `unfavorite`
mold library tag list [--json]
mold library tag add|remove <FILENAME>... --tag TAG...
mold library tag rename <OLD> <NEW>
mold library tag delete <TAG> [-y]
...
mold library collection update <NAME-OR-SLUG> [--name TEXT] [--description TEXT | --clear-description] \
                                              [--cover FILENAME | --clear-cover] [--hidden | --visible]
mold library collection delete <NAME-OR-SLUG> [-y]  # deletes the collection, never its prints
mold library collection add|remove <NAME-OR-SLUG> <FILENAME>...
mold library trash <FILENAME>...                    # move live prints into the recoverable trash
```

Organization is capability-gated per host: a server whose
`capabilities.gallery` lacks `organize` / `trash` refuses these actions by
...
| `flux2-dev:q4`                      | Slow (50 steps)   | Excellent | Full FLUX.2 Dev on a 24 GB GPU; ungated                 |
| `flux2-dev:bf16`                    | Slow (50 steps)   | Excellent | Full FLUX.2 Dev; gated, non-commercial, high host RAM   |
| `flux2-klein-base:q8`               | Slow (50 steps)   | Excellent | Undistilled 4B; true CFG + negative prompt, Apache 2.0  |
...
**FLUX.1 Fine-tunes**: `jibmix-flux:q3`, `jibmix-flux:q4`, `jibmix-flux:q5`, `jibmix-flux:fp8`, `ultrareal-v4:q8`, `ultrareal-v4:q5`, `ultrareal-v4:q4`, `ultrareal-v3:q8`, `ultrareal-v3:q6`, `ultrareal-v3:q4`, `ultrareal-v2:bf16`, `iniverse-mix:fp8`
...
**Flux.2 Klein**: `flux2-klein:bf16`, `flux2-klein:fp8`, `flux2-klein:q8`, `flux2-klein:q6`, `flux2-klein:q4`

**Flux.2 Klein-9B**: `flux2-klein-9b:bf16`, `flux2-klein-9b:fp8`, `flux2-klein-9b:q8`, `flux2-klein-9b:q6`, `flux2-klein-9b:q4`
...
**Qwen-Image-Edit**: `qwen-image-edit-2511:bf16`, `qwen-image-edit-2511:q8`, `qwen-image-edit-2511:q6`, `qwen-image-edit-2511:q5`, `qwen-image-edit-2511:q4`, `qwen-image-edit-2511:q3`, `qwen-image-edit-2511:q2` (repeatable `--image` inputs; own `qwen-image-edit` family)

**Wan 2.1 / 2.2**: `wan21-t2v-1.3b:bf16`, `wan21-t2v-14b:q5`, `wan21-t2v-14b:q8`, `wan22-ti2v-5b:fp16`, `wan22-ti2v-5b:q8`, `wan22-ti2v-5b:turbo`, `wan22-t2v-a14b:{q4,q5,q8,fp8}`, `wan22-i2v-a14b:{q4,q5,q8,fp8}` (Wan 2.1 I2V is refused — its CLIP-vision branch is unimplemented)

**MiniMax H3**: `minimax-h3-fl2va:comfy-pruned-int8`, `minimax-h3-fl2va:comfy-pruned-int8-turbo-8step`, `minimax-h3-fl2va:comfy-pruned-int8-turbo-4step-768p`, `minimax-h3-ref2va:comfy-pruned-int8`, `minimax-h3-ref2va:comfy-pruned-int8-turbo-4step` — the runnable reviewed identities. `minimax-h3-{fl2va,ref2va}:official-bf16` and `minimax-h3-{fl2va,ref2va}:comfy-pruned-nvfp4` download and inventory but refuse generation as `MINIMAX_H3_RUNTIME_UNAVAILABLE` (HTTP 501)
...
**LTX-2 / LTX-2.3 / LTX-2.5**: `ltx-2-19b-dev:fp8`, `ltx-2-19b-distilled:fp8`, `ltx-2.3-22b-dev:{fp8,bf16}`, `ltx-2.3-22b-distilled:{fp8,bf16}`, `ltx-2.5-22b-dev:{int8-conv,bf16-conv,bf16}`, `ltx-2.5-22b-distilled:{int8-conv,bf16-conv,bf16,q3-k-s,q3,q4-k-s,q4,q5,q6,q8}`
...
FLUX FP8 safetensors are automatically quantized to Q8 GGUF on first use (one-time conversion, cached at `$MOLD_HOME/cache/flux-q8/`). Qwen-Image, Wan, and LTX-2 FP8 tiers run FP8 natively and are never converted.
...
| `real-esrgan-x2plus:fp32`       | 2x    | 67 MB  | Subtle 2x (full precision)      |
| `real-esrgan-x4plus-anime:fp16` | 4x    | 8.5 MB | Anime/illustration              |
| `real-esrgan-x4plus-anime:fp32` | 4x    | 18 MB  | Anime (full precision)          |
| `real-esrgan-anime-v3:fp32`     | 4x    | 2.4 MB | Fast anime/video                |

All seven shipped upscalers, with sources and sizes: `website/models/upscalers.md`.
...
mold stats                   # Disk usage overview (models, output, logs, shared components)
mold stats --json            # Machine-readable disk usage
mold clean                   # Dry run: orphans, stale .pulling markers, hf-cache transients
mold clean --force           # Actually delete what the dry run listed
mold clean --older-than 30d --force  # Also delete output images older than 30 days
```
...
/ NSFW Civitai. Web Settings stores them on the serving host in owner-only
`$MOLD_HOME/catalog-credentials.json` (`0600`); the browser only ever receives
masked status. A saved value overrides the environment default, and clearing it
...
2. Pick the first family with **High** or **Medium** stock and enough VRAM for the requested model (`estimated_vram_need_gb`, default 18 GB) from the CLI's ascending-VRAM ladder: RTX 4090, RTX 3090, RTX 5090, L40, L40S, RTX A6000, A100 PCIe, A100 SXM, H100 NVL, H100 SXM. Nothing matching falls back to any interesting GPU with High stock and enough VRAM. (`mold_core::runpod::GPU_PREFERENCE` — 4090 > 5090 > RTX A6000 > L40S > L40 > A100 PCIe — is a separate, coarser ranking and is not what `mold runpod run` selects with.)
...
`mold lambda` deploys and operates a private, SSH-tunneled mold server on
Lambda Cloud. The API key lives in `lambda.api_key` (`mold config set
lambda.api_key <key>`) or `LAMBDA_API_KEY`.

```bash
mold lambda doctor                        # Check auth, endpoint, and account basics
mold lambda availability [--json]         # Live GPU capacity plus the selected mold image tags
mold lambda deploy --instance-type gpu_1x_a10 --region us-west-1 \
                   [--new] [--dry-run] [--json] [--forward-secrets] \
                   [--model flux-dev:q4] [--open-browser true|false]
mold lambda status [--json]               # The state-tracked instance
mold lambda logs [-f]                     # Remote mold service logs over SSH
mold lambda tunnel [--local-port 7680]    # Start or refresh the local SSH tunnel
mold lambda ssh                           # SSH into the state-tracked instance
mold lambda filesystems [--json]          # Lambda filesystems and usage
mold lambda terminate [--json]            # Terminate the instance and kill its tunnel
mold lambda reset --to-zero --confirm <TYPED>  # Delete every mold-managed Lambda resource
```

`deploy` reuses the state-tracked instance unless `--new` is passed,
`--dry-run` prints the launch request and exits, `--forward-secrets` copies the
local `HF_TOKEN` / `CIVITAI_TOKEN` into the remote service environment, and
`--model` enqueues that download once the tunnel is ready. `--open-browser`
defaults to true.
...
mold server start --log-file         # Rotated file logging (already on by default here; opt-in for `mold serve`)
...
particular job is durable (a job with no gallery target or one whose request
exceeds the payload ceiling runs normally but is not replayed).
...
can be replayed after a server restart.

### HTTP API Endpoints

Core endpoints exposed by `mold serve` (full list + schemas at `/api/docs`):

- **One admission path.** `/api/generate`, `/api/generate/stream` and `/api/generation-batches` all admit through the durable queue. A host that cannot — `MOLD_DB_DISABLE=1`, gallery output off, no queue journal, a missing admission service, or `MOLD_DISPATCH_MODE=legacy|observe` — refuses all three with `503 DURABLE_ADMISSION_UNAVAILABLE`, and `capabilities.queue.heterogeneous_batch_max_outputs` is absent. A LoRA beside conditioning media is an ordinary durable request (persisted, re-validated at dispatch, held by name if the adapter vanished). Two traits refuse with a typed `422`: ordered H3 references (one-use upload authority the queue cannot persist) and `hdr_exr_dir` (an output directory on the inference machine — re-run with `--local`)
- `POST /api/generate` — one output, raw bytes response. A facade over the SAME durable admission `/api/generation-batches` uses, with an attached raw observer, so `batch_size` must be 1. A print that fails while the caller is attached is the caller's error in the singleton contract's own shape (404 `MODEL_NOT_FOUND`/`UNKNOWN_MODEL`, 503 `QUEUE_FULL`, else 500 with the engine's sentence) naming the held job to resume; `202` with the `GenerationBatchStatus` is answered only when the observer detached. Optional `X-Mold-Client-Batch-Id` (a UUID) is the batch's `client_batch_id`: a replay answers 200 with the admitted batch status as JSON instead of a second render
- `POST /api/generate/stream` — the same admission with an attached SSE observer: progress, denoise previews, and a base64 complete event. A terminal error frame carries the server's own code, so a `MODEL_NOT_FOUND` / `UNKNOWN_MODEL` hold still drives client auto-pull
- `POST /api/generation-batches` — durable idempotent 1..N admission (`batch_size: 1` per child) on any host that generates; recover with `GET /api/generation-batches/by-client/:client_batch_id`, poll with `GET /api/generation-batches/:id`, reconcile sets with `POST /api/generation-batches/status`, and cancel a whole print run with `DELETE /api/generation-batches/:id` (the per-child cancel applied to every non-terminal child; settled children keep their outcome)
- Every batch child carries a monotonic `revision`, bumped by each authoritative state transition. Order snapshots and events by it, never by `updated_at_ms`: transitions collide inside a millisecond, and a retry moves a child backward (`held` → `accepted`), so a timestamp tie-break silently drops the retry. The field is additive — absent, or `0`, means "no revision authority" (an older host, or a row not yet transitioned since the migration) and callers fall back to the timestamp. After a retry POST whose response was lost, compare the child's revision against the one captured before the POST: still `held` at a HIGHER revision means the retry landed and the job re-held; still at the submitted revision means it never landed, so keep the retry fence rather than republishing the stale snapshot.
- `POST /api/generate/placement-preview` — legacy read-only Scheduler V2 projection for `{ request, copies }` (`copies` is `1..=64`). Canonical protocol-v2 pinned admission does not call it, and automatic protocol-v2 routing uses cached telemetry rather than a placement probe. Legacy ordinary generations return a version-1 authoritative candidate only when the request is admissible; the call never leases a device, loads a model, starts a download, or mutates queue state. `planned` may include additive `pending_downloads` for known encoder dependencies selected by that candidate plan, using a preview-only registry identity and low-confidence estimate until admission lands and re-fingerprints the file. Cold installed `cv:`/`hf:` IDs resolve only from contained local sidecars, and their synthesized runtime config remains attached to prepared work through planning and pre-CUDA validation across config refreshes. `infeasible` may include additive `missing_components` with `repair_model`. Legacy clients queue nothing on infeasible and only offer repair/resume when they own the exact host's complete grouped pull. A legacy prepared Batch N preview sends one sibling-shaped request (`batch_size: 1`) with `copies: N`.
- `POST /api/chain-jobs` — chained arbitrary-length video (LTX-2, LTX-Video, Wan); additive `ephemeral: true` marks an auto-chained one-shot (absent from authored sequence listings and `chain_job_queued`, visible in `/api/activity`, parked with all media on restart, explicitly resumable, reclaimed by cleanup only once settled, and published with no `chain_job_id` so Reuse settings restores a one-shot); accepts every video `output_format` (the job's own artifact is stitched as MP4 and the gallery print is transcoded to the requested mp4/gif/webp/apng at finalization; `finalized.gallery_filename` names it); body is `mold_core::chain::ChainRequest` (canonical `stages[]` or auto-expand `prompt`+`total_frames`+`clip_frames`); returns a job id, and `GET /api/chain-jobs/{id}/events` streams stage progress to settlement. The synchronous and SSE `POST /api/generate/chain[/stream]` compatibility endpoints are gone: they ran a chain as a hidden ephemeral job that could not be resumed, retaken, or reattached
...
- Durable chain jobs:
  ...
  - `POST /api/chain-jobs/:id/amend` — edit a settled/queued sequence in place: body is `AmendRequest` (the FULL edited `stages[]` plus optional `motion_tail_frames`/`fps`/`seed`/`steps`/`guidance`/`strength`/`enable_audio` overlays; model, size, output format, placement, and batch provenance are NOT amendable). A changed `strength` is a chain-level invalidator, like seed/steps/guidance/fps/motion_tail_frames. Returns 202 `AmendResponse` (flattened `ChainJobSummary` + `preserved_stages`) and requeues from the earliest dirty clip; `cut`↔`fade` and fade-length edits re-finalize with zero re-renders
...
- `GET /api/gallery[?view=library|trash][&filename=NAME]` (`filename` narrows the listing to one print — use it instead of reading the whole index to resolve one row's metadata; a path-shaped value is refused rather than silently matching nothing) · `POST /api/gallery/media-token` · `GET /api/gallery/image/:name` · `GET /api/gallery/thumbnail/:name` · `DELETE /api/gallery/image/:name[?permanent=true]` (trash by default; `permanent` hard-deletes) · `PATCH /api/gallery/image/:name` (`{title?, favorite?, tags?, add_tags?, remove_tags?}` → updated `GalleryImage`)
...
- Trash (`capabilities.gallery.trash = { enabled, retention_days }`): `POST /api/gallery/trash` / `POST /api/gallery/trash/restore` (`{filenames}`; restore is `409 GALLERY_RESTORE_CONFLICT` when a live print holds the name) · `DELETE /api/gallery/trash` → `{purged}` · `POST /api/gallery/trash/delete-forever` (`{filenames}` — bulk permanent purge of selected trashed prints) · `POST /api/gallery/trash/sweep` → `{purged, remaining}`; rows from `?view=trash` carry `trashed_at` / `purge_at`. SSE adds `gallery_updated`, `gallery_trashed`, `gallery_restored`, `gallery_collections_changed`; purge reuses `gallery_removed`
...
- `DELETE /api/queue/:id` — cancel a queued **or running** generation job (204; 404 unknown or already completing). Running inference stops cooperatively at the next model safe point; the server advertises this as `capabilities.queue.cooperative_cancellation`. There is no 409 arm.
...
| `MOLD_DB_DISABLE`                     | unset                                  | `1` opts out of the `mold.db` metadata database entirely; CLI, server, and TUI fall back to filesystem-only behaviour and every organize/trash/history capability reports off |
| `MOLD_DB_PATH`                        | `$MOLD_HOME/mold.db`                   | Absolute path override for the metadata database file |
| `MOLD_PROFILE`                        | `default`                              | Active settings profile — `settings` and `model_prefs` are keyed on `(profile, key)`; overrides the stored `settings.profile.active` |
| `MOLD_GPUS`                           | unset (all visible)                    | GPUs for local generation on `mold run` / `mold serve`: `all`, `none` (fail-closed maintenance mode), visible ordinals, or stable `cuda:<uuid>` / `metal:default` / `GPU-…` / `MIG-…` IDs |
| `MOLD_MDNS`                           | on                                     | `0` / `false` / `no` / `off` disables mDNS/DNS-SD advertising and server-assisted browsing (same as `mold serve --no-mdns`); any other value keeps it on |
| `MOLD_QUEUE_SIZE`                     | `200`                                  | Jobs hydrated into the runtime queue window. The durable backlog itself is uncapped — this is not a limit on admitted work |
| `MOLD_ARTIFACT_ATTESTATIONS_DIR`      | `$MOLD_HOME/.artifact-attestations-v1` | Owner-private durable digest attestations for pinned artifacts; set this to private state outside a shared/group-writable `MOLD_HOME` so unchanged models are not rehashed after restart                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `MOLD_THUMBNAIL_WARMUP`               | enabled                                | Enabled by default; set `0` to skip startup thumbnail warmup                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `MOLD_LOG`                            | `info` (server) / `warn` (other)       | Log level (trace/debug/info/warn/error). `mold serve` and `mold server start` fall back to `info` and read `[logging] level`; every other command falls back to `warn`, and the TUI logs `warn` to file only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `MOLD_HOST_RAM_ZFS_ARC`               | on                                     | `0` / `off` / `false` / `no` stops counting evictable ZFS ARC as host-RAM headroom. On a Linux host running OpenZFS, admission adds OpenZFS's own reclaimable figure (`min(Σ mru/mfu evictable, size − c_min)` from `/proc/spl/kstat/zfs/arcstats`) beside `MemAvailable`, because the kernel never counts the ARC there even though its shrinker drains it inline; the credit is zero while `memory_available_bytes` is negative, `zfs_arc_pc_percent` is non-zero, or `zfs_arc_shrinker_limit` is non-zero/unreadable (OpenZFS 2.2 default — set it to 0). Refusals, `/api/status.host_memory.reclaimable_zfs_arc_bytes`, and the Machines pages name the credit when it is above zero. Disable it in a container that sees the host's ARC but is limited by its own cgroup.                                                        |
...
| `MOLD_ATTN`                           | video `flash`, images `math`           | Attention backend: `math` (hand-rolled SDP) or `flash` (candle-flash-attn v2; needs a `--features cuda,flash-attn` build and a CUDA fp16/bf16 tensor). Default is per family: video DiTs (Wan, LTX-2) take `flash` where the kernel is compiled (measured 2.1x on Wan), image families stay `math` so an archived still's bytes never depend on the artifact. Setting the variable overrides both. Ineligible tensors fall back to math silently; a build without the feature warns once.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `MOLD_CONV`                           | video `cudnn`, images `im2col`         | Convolution backend: `im2col` or `cudnn` (needs a `--features cuda,cudnn` build; every Linux CUDA artifact is one). Default is per family on the same argument as `MOLD_ATTN`: video families take `cudnn` (measured 4.4x on the Wan VAE decode's convolutions), image families stay `im2col` so an archived still's bytes never depend on the artifact. candle keeps 1x1 and small convolutions on im2col regardless, and falls back to it on any cuDNN error. |
...
| `MOLD_LTX2_GEMMA_VARIANT`             | `auto`                                 | LTX-2 Gemma 3 12B weight format: `auto` — on the server path this is memory-aware: BF16 wins only when its shard bytes plus a host-headroom reserve (max(15% of total, floor) + 256 MiB) still fit in available host memory, otherwise exactly one present GGUF is chosen and the downgrade is logged; off macOS, with no GGUF, or with an ambiguous GGUF set it stays BF16. The forced-local fallback in `ltx2/text/gemma.rs` remains presence-only (BF16 if present, else GGUF), `q4` (force Q4 GGUF — `google/gemma-3-12b-it-qat-q4_0-gguf`, ~7 GB; fits comfortably on a 24 GB card alongside the streaming transformer), `bf16` (force BF16 split — `google/gemma-3-12b-it-qat-q4_0-unquantized`, ~23 GB; historical default). Auto-detection scans the gemma_root for `*.gguf` and `model*.safetensors`. Place the Q4 GGUF manually in your gemma_root for V1 — manifest auto-fetch is deferred.                                                                                                                                                                                                                                                                                                                        |
...
| `MOLD_LTX2_INT8`                      | `native`                               | LTX-2.5 `int8-conv` transformer linear execution on CUDA. `native` (default) keeps the packed INT8 weights resident on the device and runs ComfyUI's exact W8A8 order (rotate activation → dynamic per-row INT8 → cuBLASLt INT8 GEMM → both F32 scales, bias after the GEMM). `dequant` widens each linear to the activation dtype per forward through the fused ConvRot kernel (W8A16) for A/B comparison — one transient dense linear at a time, never a block. Metal/CPU ignore it and keep the widening arm. Logged once as `ltx2 int8 arm=…`; fingerprint-registered so the two arms never share a learned estimate.                                                                                                                                                                                                                |
| `MOLD_LTX2_QMATMUL`                   | `0`                                    | Quantized (GGUF) LTX-2.5 transformer linears on CUDA. Experimental, off by default: `1`/`true`/`on`/`yes` feeds candle's MMQ/MMVQ kernels, the same kernels that NaN for Qwen-Image and render Z-Image solid black (#1048). Default dequantizes each block linear per forward at the LTX compute dtype; Metal and CPU ignore it (Metal keeps `QMatMul`, CPU always dequantizes). Logged once as `ltx2 linear kind=qmatmul` or `ltx2 linear kind=dequant`; fingerprint-registered so the two arms never share a learned estimate.                                                                                                                                                                                                                                                                                                    |
...
| `MOLD_WAN_STEP_CACHE`                 | `auto`                                 | Wan first-block residual reuse (#801) on the non-distilled tiers: `off`, `auto` (threshold 0.10, measured 1.85x on the A14B `:q8` tier), or an explicit positive relative-L1 threshold. **On by default**; refused with a message on distilled or sub-12-step schedules, so it only engages where it was qualified. `off` is bit-identical to denoising every block.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `CIVITAI_TOKEN`                       | unset                                  | Civitai token for early-access / NSFW catalog downloads                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- FLUX FP8 safetensors auto-convert to Q8 GGUF on first use (fits 24GB cards); other families run FP8 natively
...
- `/sequence <prompts> [model] [frames_per_clip] [width] [height] [fps] [steps] [guidance] [seed] [audio]` — render a durable multi-prompt video sequence; `prompts` is 2–16 clip prompts separated by `|`, and `model` autocompletes to sequence-capable video models
...
defaults fresh installs to Safelight + **Dark** with Photos auto-save and
"Tag new prints with their title" (`autoTagTitle`) enabled while retaining valid
saved choices, and synchronizes UIKit's appearance/status bar. Canonical:
`website/guide/iphone.md`. Its default-on Photos option fetches completed
...
android-dev        # run the Android app with Tauri hot reload
android-run        # build and run the production app on Android
android-check      # build a debug ARM64 APK from the shared mobile shell
android-test       # build Android and run native instrumentation tests on an emulator
android-build      # build Android ARM64/ARMv7 app bundles for Google Play
...
desktop-release    # signed, notarized macOS distribution build
```

Android ships from the same remote-only Tauri crate and `desktop/src/mobile`
