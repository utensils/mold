...
- `POST /api/generate` — one output, raw bytes response. A facade over the SAME durable admission `/api/generation-batches` uses, with an attached raw observer, so `batch_size` must be 1 and an accepted job that then fails answers `202` with its `GenerationBatchStatus` rather than a `500`
