...
- Server-owned batch planning reads one static `mold-inference` family registry before engine load, constructs every advertised alias in tests, and validates the instantiated runtime against it. Every production family currently declares only native size `[1]`; initial-wave cardinality is lexically first, homogeneous singleton planning is exact at arbitrary size, and host-coupled heterogeneous singleton planning is exact through 8 devices at arbitrary child count. Singleton results retain one arithmetic lane per selected device with typed random access and bounded 4096-record windows. Ordinary native inputs expose the deterministic 64-strategy bounded heuristic; huge batches use the mandatory compact singleton fallback as `BoundedHeuristic`. On protocol-v2 hosts every remote 1..N generation is admitted as ordered singleton children through `/api/generation-batches`, reconciled by client/batch ID, and chunked above the advertised heterogeneous limit. Direct singleton `/api/generate[/stream]` remains the explicit compatibility fallback for older or request-ineligible hosts. No engine is represented as returning multiple native outputs; old raw-parent records are supported only during restart recovery.
