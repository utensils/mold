...
/// kernel claim, and `omitted:compiled`, which is the shipping shape of the
/// Linux CUDA compute capabilities QUALIFIED for flash: sm86 and sm100 name
...
/// and none of the speedup. sm120 is deliberately NOT among them and ships
/// `omitted:omitted` — FA2's runtime `is_sm8x` tile selection reads consumer
/// Blackwell as a datacenter part, and nobody has measured the result (see
/// `flashAttnQualifiedCaps` in flake.nix) — so all four markers are legitimate
/// shipping shapes. What stays forbidden in a published artifact is the
/// H3-SCOPED developer kernel — the claim marker and the public Qwen support
/// loader on a build whose provenance says `h3-rc=omitted` — never the global
/// dispatch on its own. The default attention backend stays `Math` for
/// `AttentionPolicy::Image` in every build (#736).
