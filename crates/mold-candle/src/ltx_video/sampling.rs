// LTX Video FlowMatchEulerDiscreteScheduler
// Ported from FerrisMind/candle-video (Apache 2.0)
// Original: https://github.com/FerrisMind/candle-video

//! FlowMatchEulerDiscreteScheduler (Euler, discrete) for LTX Video.
//!
//! Standalone implementation — no external scheduler trait dependencies.
...
impl FlowMatchEulerDiscreteScheduler {
    pub fn new(config: FlowMatchEulerDiscreteSchedulerConfig) -> Result<Self> {
        ...
        if config.use_beta_sigmas {
            bail!("use_beta_sigmas is not supported (requires statrs dependency). LTX v0.9.5 does not use beta sigmas.");
        }
    ...
    pub fn set_timesteps(
        ...
        // 4) Optional conversion to karras/exponential
