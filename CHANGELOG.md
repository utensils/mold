...
- **LTX-2 chain continuations no longer crash with `conditioning replacement exceeds video token count` on the second stage.** The distilled pipeline's `prepare()` implicitly halves stage-1 linear resolution (see `pipeline_uses_two_stage_spatial_refinement` — the distilled LTX-2 path bakes in a built-in X2 spatial refinement, runs the denoiser at half resolution, and spatially upsamples the rendered RGB to `plan.width × plan.height` before returning it to the caller). Continuation stages then carry the prior clip's *full-resolution* RGB tail into `maybe_load_stage_video_conditioning`, which VAE-encoded those frames as-is into latents with 4× too many spatial tokens (2× linear) relative to the receiving clip's half-resolution token grid — and `apply_video_token_replacements` refused to splice an oversized replacement into the shorter main sequence. The bug was latent at `motion_tail_frames = 17` because `3 latent frames × (upscaled 38×22) = 2508 tokens` still fit under the `13 × 19 × 11 = 2717` main token count (just barely, and the token contents themselves were also spatially wrong), but the bump to `motion_tail_frames = 25` (four latent frames × 836 = 3344) tipped it past the main budget and surfaced the crash. Fixed in `crates/mold-inference/src/ltx2/runtime.rs::maybe_load_stage_video_conditioning` by resizing every staged tail frame to the receiving clip's `pixel_shape.width × pixel_shape.height` via `imageops::resize(Lanczos3)` before feeding them to `video_tensor_from_frames` and the VAE encoder. The `conditioning.images` path is unchanged because it already routes through `decode_source_image` with an explicit target size.

### Changed

- **LTX-2 chain continuations now reuse the prior clip's last decoded frame as the soft identity anchor instead of the original user upload, and hold onto more pixel context at each stitch.** `Ltx2ChainOrchestrator::run` (`crates/mold-inference/src/ltx2/chain.rs`) now PNG-encodes the last RGB frame of each stage's carry and overwrites the *next* stage's `source_image` before dispatching to the engine, so the renderer's existing soft-anchor routing in `Ltx2Engine::render_chain_stage` lands on a reference that reflects current scene state rather than an increasingly-stale original upload. Previously, `build_auto_expand_stages` replicated the original upload across every stage and `render_chain_stage` routed it to the soft-anchor slot at frame=motion_tail with `CHAIN_SOFT_ANCHOR_STRENGTH = 0.4` — the 0.4 soft-pull kept yanking drifted continuations back toward the starting image, producing the "each clip is a different video" effect users reported on long render-chain-v1 runs (for pure text-to-video chains the soft anchor wasn't populated at all on continuations, which hurt identity coherence for a different reason). Now that the anchor is always fresh and meaningful, `CHAIN_SOFT_ANCHOR_STRENGTH` moves 0.4 → 0.6 (denoiser now blends ~40% generated / ~60% anchor at the anchor token every step), and `DEFAULT_MOTION_TAIL` moves 17 → 25 pixel frames — four LTX-2 latent frames of hard-pinned carryover (causal-first + three continuation slots, ≈1.04 s at 24 fps) for a longer motion lead-in at each stitch. Kept in sync across `web/src/lib/chainRouting.ts`, the `--motion-tail` CLI flag default in `crates/mold-cli/src/main.rs`, and the `ChainRequest` serde default in `crates/mold-core/src/chain.rs`. New orchestrator tests `chain_rewrites_continuation_source_image_to_prior_last_frame` and `chain_without_source_image_still_rewrites_continuations_from_carry` pin the rewrite behaviour; the existing `chain_propagates_source_image_to_every_stage` invariant (every stage has a source image set) still holds.
