 ...
 * fallback staying honest as "Join clips" (LTX-Video carries no motion
 * across the seam, so "Smooth" would over-promise). Lives in ui/ so the
...
/** LTX-2's context handoff length; LTX-Video uses 0 ("Join clips"). */
...
export function transitionLabel(
  ...
  if (transition === "smooth") {
    return motionTailFrames > 0 ? "Smooth" : "Join clips";
  }
