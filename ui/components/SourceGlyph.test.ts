...
describe("SourceGlyph", () => {
  ...
  it("sizes width and height from the size prop, defaulting to 12", () => {
    const defaultSize = mount(SourceGlyph, { props: { source: "hf" } }).get("svg");
    expect(defaultSize.attributes("width")).toBe("12");
    expect(defaultSize.attributes("height")).toBe("12");

    const sized = mount(SourceGlyph, { props: { source: "hf", size: 16 } }).get("svg");
    expect(sized.attributes("width")).toBe("16");
    expect(sized.attributes("height")).toBe("16");
  });

  it("gives Civitai's gradients document-unique ids across two mounts", () => {
    ...
    const ids = wrapper.findAll("linearGradient").map((n) => n.attributes("id"));
