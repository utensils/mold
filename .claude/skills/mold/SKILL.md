...
Signed builds also expose **Settings → Updates** with persisted **Stable** and **Nightly** channels. Startup performs a best-effort check only; the menu and Settings offer the same manual check, and nothing downloads or installs until the user chooses **Update and restart**. Stable follows tagged releases through the public `mold-desktop-stable.json` manifest. Nightly follows signed, notarized builds from desktop-relevant `main` commits through the rolling `mold-desktop-nightly.json` manifest. Tauri must verify the updater's Minisign signature, and Mold must bind the signed archive plus installed bundle ID/version to the manifest before launch. Mold keeps a persistent backup and runs its old binary as an external supervisor: the frontend has 15 seconds to paint and signal readiness after preferences and restored routing, without waiting for engine or remote-host startup, then the candidate must survive 10 seconds of probation. Interrupted installs reconcile on the next launch; failures restore/relaunch the backup with a visible recovery error, while rollback failure preserves and reports its manual recovery path. Selecting Stable from a newer Nightly does not downgrade immediately; it waits for a newer stable version.
