...
Desktop sequence authoring now lives in Create's **Output = Sequence** setting: the clip rail replaces the composer, seam pills open the transition editor, and sequence jobs share the Create activity strip with prints. Legacy chain URLs redirect to `/create?output=sequence`.
