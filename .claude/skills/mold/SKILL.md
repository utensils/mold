...
| `MOLD_KEEP_TE_RAM`                    | auto (CUDA) / off                 | `1` forces text encoders to park on CPU host RAM between requests; any other value forces drop-and-reload. Unset, Qwen-Image defaults on for CUDA builds when `MemAvailable` covers the encoder's bytes plus an 8 GiB margin (#1044, removes a 35.1 s per-request reload); other families stay opt-in. Quantized GGUF encoders park too — their `QTensor` bytes move losslessly instead of being re-read from disk. Disabled on Metal. Covers Wan's UMT5-XXL, whose parked F16 copy saves an 11.4 GB disk read per consecutive render. |
