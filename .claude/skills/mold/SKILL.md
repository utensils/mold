...
- Server-owned batch planning reads one static `mold-inference` family registry before engine load and validates the instantiated runtime against it. Every production family currently declares only native size `[1]`; the pure adaptive planner and future `BatchChild` projections do not mean raw `batch_size > 1` is routed or that `server_batch` may be advertised yet.
