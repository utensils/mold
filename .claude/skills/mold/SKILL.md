...
| `MOLD_KEEP_TE_RAM`                    | unset                             | Set `1` to park text encoders on CPU between requests instead of dropping them (FP16/BF16 only; GGUF falls through to drop+reload). Disabled on Metal. Covers Wan's UMT5-XXL, whose parked F16 copy saves an 11.4 GB disk read per consecutive render.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `MOLD_ATTN`                           | `math` (`flash` with the feature) | Attention backend: `math` (hand-rolled SDP) or `flash` (candle-flash-attn v2; needs a `--features cuda,flash-attn` build and a CUDA fp16/bf16 tensor). That build defaults to `flash`; every other build defaults to `math`. Ineligible tensors fall back to math silently; a build without the feature warns once.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `MOLD_WAN_OFFLOAD_BLOCKS`             | auto                              | Trailing Wan DiT blocks to park in host RAM (#776). Auto-engages only when the render will not otherwise fit; `0` disables. GGUF only. 81f/832x480 A14B q5: 316.3 s at 17,322 MiB, previously OOM. Trades wall clock for VRAM. |
