...
describe("ControlsAside", () => {
  ...
  it("renders the host's own note for a bounded but adjustable step control", () => {
    ...
    // the control still moves. The sentence has to render anyway — a bound
    // the user can see and cannot account for reads as a bug.
    ...
    // And the control it explains is still usable.
    const steps = wrapper
      .findAllComponents(SliderRow)
