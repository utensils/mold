/**
 * ↑/↓ prompt-history cycling for the Create composer, shell-history style:
 * ↑ walks from the newest recorded prompt toward older ones, ↓ walks back and
 * finally restores the draft the user was typing. Pure state machine — the
 * view feeds it entries from GET /api/history and caret-position checks.
 * Ported from the desktop app (desktop/src/lib/promptCycler.ts) so both
 * surfaces recall prompts identically.
 */
export class PromptCycler {
  ...
  setEntries(prompts: string[]) {
    ...
    // Entries changing mid-navigation (a refetch) would make the cursor point
    // at a different prompt than the one on screen; start over.
    this.cursor = null;
  }

  /** Make an accepted submission immediately available while the server
   * persists the row and the next refetch catches up. */
