...
3. Every mutation first reads server capabilities. `gallery.organize=false`
...
4. Recoverable removal is `mold library trash` and requires an advertised,
...
3. Extract the existing terminal preview entry point for reuse.
4. Add parser/unit tests plus black-box WireMock coverage for request method,
   ...
   capability fallback, confirmations, partial
...
- Default-feature and shipping-feature builds compile; focused CLI and core
