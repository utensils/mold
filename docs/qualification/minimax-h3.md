...
- Runtime status: **compact FL2VA and Ref2VA publicly available on supported
  SM89 CUDA and Apple Silicon Metal; each request remains gated by its exact
  device and host-memory budget**
- Private qualification status: **authorized under the external private-UAT record**
- Evidence snapshot: **2026-09-20, Metal UAT binary
...
| Metal | The supported shipped path includes bounded chunked dense attention, fused last-dimension softmax, portable quantized linears, and streamed Qwen/DiT blocks. The [2026-09-05 memory qualification](minimax-h3-metal-memory.md) reproduces and fixes attention and INT8 pooled-buffer retention; the [current campaign](minimax-h3-metal-next-campaign.md) adds exact per-request budgets, external pressure/swap/floor enforcement, and a guarded 256×256, 107-frame FL2VA Turbo 4-step render with stereo audio on a 48 GiB Mac. | Supported requests remain subject to exact live-memory admission. Default-resolution fit, broader modes, and full cross-backend output comparison are retained evidence boundaries, not hidden client gates ([#1164](https://github.com/utensils/mold/issues/1164), [#1542](https://github.com/utensils/mold/issues/1542)). |
| Server/factory      | Pinned identities, upstream-direct compact acquisition, runtime request contracts, secure reference ingress, prepared shapes, frozen single-GPU admission, block-streaming ownership, immutable factory authority, fail-closed backend adapter, and FL2VA/Ref2VA worker dispatch. The compact family advertises `Supported` on CUDA and Metal; every request still crosses exact device and host-memory admission. | Official BF16 and pruned NVFP4 identities remain download-only because no public engine arm executes those layouts. |
| Studio surfaces     | [#867](https://github.com/utensils/mold/pull/867) provides shared web, desktop, and iPhone authoring, recovery, canonical upload, and provenance contracts. The shared inventory parser accepts the server's exact `cuda-or-metal` qualification and renders supported Metal hosts instead of dropping their capability; targeted Studio, web, and mobile suites cover the same fixture. | Host-authored `runtime_available`, request-envelope hash, installed components, and live reachability remain mandatory; no client manufactures support. |
| CLI/TUI/Discord     | [#868](https://github.com/utensils/mold/pull/868) provides ordered-reference authoring, canonical reference leases, and media provenance. CLI forced-local and cold-server routes execute the same supported Metal runtime; Discord filters only rows the server marks unavailable. | Forced-local remains one FL2VA request; batches, chains, and Ref2VA uploads use the server route. |
| Real checkpoint     | [PR #883](https://github.com/utensils/mold/pull/883) authenticated both released 20.97 GB INT8 task transformers from retained descriptors, validated the shared 932-tensor header and 200 quantization sidecars, executed isolated block 0 on CUDA, and returned to zero live blocks; both reports recorded `factory_activated: false`. | That historical fixture proves only its isolated CUDA block. End-to-end public support is established by the later CUDA and Metal campaign rows above, not retroactively by this block result. |
