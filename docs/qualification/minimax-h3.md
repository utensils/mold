...
- Runtime status: **compact FL2VA publicly available on the supported SM89 CUDA
  route; Apple Silicon Metal has a retained reduced-size FL2VA smoke render,
  while default-resolution qualification remains open**
...
| Metal | The shipped correctness path includes chunked dense attention, portable quantized linears, and streamed Qwen/DiT blocks. The [2026-09-05 memory qualification](minimax-h3-metal-memory.md) reproduces and fixes attention and INT8 pooled-buffer retention, and retains a guarded 256×256, 107-frame FL2VA Turbo 4-step render with stereo audio on a 48 GiB Mac. | `CorrectnessOnly` remains: reduced-size server-route smoke evidence exists; default-resolution fit, broader modes, and full output parity remain separate acceptance work ([#1164](https://github.com/utensils/mold/issues/1164), [#1542](https://github.com/utensils/mold/issues/1542)). |
