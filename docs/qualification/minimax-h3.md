...
- Evidence snapshot: **2026-08-07 19:31 UTC, Mold main `b5ea538a`**
...
| Merged on main                 | Memory-efficient full attention, ordered Ref2VA orchestration, portable Comfy quantization, fail-closed backend adapter, per-layer comparator, frozen factory authority, FL2VA dispatch, scaled FP8/Qwen INT8 execution, and Ref2VA dispatch                                          | [#857](https://github.com/utensils/mold/pull/857), [#859](https://github.com/utensils/mold/pull/859), [#860](https://github.com/utensils/mold/pull/860), [#861](https://github.com/utensils/mold/pull/861), [#862](https://github.com/utensils/mold/pull/862), [#863](https://github.com/utensils/mold/pull/863), [#864](https://github.com/utensils/mold/pull/864), [#865](https://github.com/utensils/mold/pull/865), and [#866](https://github.com/utensils/mold/pull/866); main `b5ea538a305a0b46bdfaf483baba3f46819da720`                                                                                                                                                                                                               | Both task dispatch seams exist, but frozen `runtime_available = false` authority rejects activation before a real load      |
| Pending review                 | Web, desktop, and iPhone authoring, gated inventory/readiness, durable draft media, canonical reference-upload probing, exact recovery identity, and closing-frame provenance                                                                                                         | [#867](https://github.com/utensils/mold/pull/867), exact head `6b7fbc0a23091db7c5c8b4b4b6395d28c5e5890d`. At the evidence timestamp GitHub reported it open and mergeable but blocked: the `rust` job failed in its Test step and `nix-web` was pending; the other applicable reported checks had passed or were skipped.                                                                                                                                                                                                                                                                                                                                                                                                                    | No qualification effect unless merged; even when merged, the UI remains fail closed while runtime capability is unavailable |
| Stacked remote PR              | CLI, TUI, and Discord ordered-reference authoring and media provenance                                                                                                                                                                                                                | [#868](https://github.com/utensils/mold/pull/868), remote head `4deb1fe56ce17398cabc2a9e8f16eade56f70a52`, based on #867. GitHub reported it open and conflicting/dirty at the evidence timestamp; its reported macOS and Linux Rust checks passed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | No qualification effect unless rebased, reviewed, and merged; no runtime activation                                         |
| Unpublished local surface work | Additional canonical-upload and ordered-reference changes on clean local head `e7d6193677394049a49baa7ef547a9b9f86c4f28`                                                                                                                                                              | The local `feat/h3-tui-discord-references` worktree was not the #868 remote head and had diverged from its tracking branch. It is a local development snapshot, not PR or CI evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | No qualification effect; do not attribute these changes or any local test result to #868                                    |
| Local attention candidate      | Isolated H3 FlashAttention feature, synthetic qualification probe, CI contract, and explicit release-binary exclusion                                                                                                                                                                 | Base commit `0204e9d983098a4c8dfc196e42dd1d87d2ee8994`. The inspected `perf/h3-attention-release-candidate` worktree was also 2 commits behind current main and had uncommitted hardening changes in 16 files; those edits are not a reviewable or reproducible Git snapshot.                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Not merged, not compiled into a published Mold binary, and not real H3 UAT                                                  |
...
The ledger is a point-in-time audit, not a merge plan. A local worktree state
cannot repair a remote PR, and a PR check result cannot qualify a model. In
particular, the exact #867 head was not all-green, the #868 remote head was
conflicting, the newer local surface work was unpublished, and the attention
candidate contained uncommitted edits when inspected.

An older clean `feat/h3-cli-tui-discord` worktree at
`b087999832d16d88d6ec5566bd0f3eeb3c8c889d` was 9 commits behind current main.
It is not the #868 remote head and contributes no additional evidence to this
record.
...
| CUDA full attention | [PR #857](https://github.com/utensils/mold/pull/857) records 121 synthetic H3 tests on the Plato host (4× L40S, SM89, CUDA 12.8), including BF16 dense-reference-to-FlashAttention-v2 parity. The recorded run for unmerged base commit `0204e9d9` separately passed 141 synthetic tests, compiled 53 compute-89 FlashAttention-v2 objects, and exercised a synthetic 257-row BF16 56×128 CUDA dispatch shape in 4,745 microseconds with maximum absolute delta `0.000488` against a `0.02` bound. Its linear LSE auxiliary accounting was 8.35 MB at 37,296 rows and 24.16 MB at 107,856 rows, with zero bytes of quadratic score/probability tensor allocation. The report explicitly recorded `model_artifacts_accessed = false` and `runtime_activated = false`. The later uncommitted hardening edits were not requalified by this audit. | Synthetic development evidence only; the candidate is release-excluded and these measurements do not establish real-model correctness, quality, peak memory, or throughput |
...
| Studio surfaces     | Exact head `6b7fbc0a` has local frontend, server-upload, MP4/AAC media-arithmetic probe, formatting, and warnings-denied clippy evidence. The MP4/AAC fixture is ordinary generated test media, not H3 output. At the timestamp above, [#867](https://github.com/utensils/mold/pull/867) had a failing `rust` Test step and pending `nix-web`, so there is no exact-head all-green claim.                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Authoring and recovery contracts only; gated readiness cannot make the runtime available                                                                                   |
| CLI/TUI/Discord     | [#868](https://github.com/utensils/mold/pull/868) records weight-free surface tests at remote head `4deb1fe5`, but GitHub reported that stacked head conflicting. Clean local head `e7d61936` contains further surface changes and is not the remote PR head; this audit did not convert its local state into PR or CI evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Authoring and provenance contracts only; no runtime activation                                                                                                             |
