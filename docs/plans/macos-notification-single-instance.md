...
Hosted CI then exposed two existing call-entry races in separate attempts: `aPlacementPreviewSendsARedactedRequest` inspected an argument before the fake captured it, and `theMenuAndTheContextualMenuCallTheSameThing` inspected loaded models before the second capture. The follow-up changes only their wait predicates to the captured state they assert, following the existing `settle` contract and neighboring placement-copy test. Assertions and wait budgets remain unchanged.
