...
LTX Video, LTX-2, and Wan all default to MP4. This lets LTX-2 preserve
...
its `17n+5` grid. It renders one clip per request. See
...
| Library   | Unified local/remote grid with persisted top-bar thumbnail sizing, host filters, deduplication, batch provenance, a Runs/Prompts History column, and desktop file actions                                                                         | Merged saved-host library with persisted pinch-to-resize thumbnail sizing, Prints/Collections/Trash scopes, favorites/tag chips, titles via the viewer Info sheet, per-host trash retention on host detail, batch/source provenance, full-screen image/video, swipe navigation, Use as prompt, and Use as source |
| Styles    | Ready to use / Browse more with the Still picture · Short clip · 3-D object chips, friendly names over exact ids, kind and explicit 18+ NSFW badges, Get it onto any machine still missing the style (Repair once all have it), download progress | Installed/live union, matching kind/18+ badges and detail, same install-or-repair host picker, pull progress, cancel, load/unload/remove                                                                                                                                                                         |
...
| Settings  | A 200px jump nav beside one scrolling page of sixteen always-open sections from the shared schema (Cloud GPUs and Per-style defaults included); rows save on change; `?section=` deep links                                                       | Six Mold Studio themes with Match phone, remote-host shortcut, version, and TestFlight update channel                                                                                                                                                                                                            |
| Sequences | Not authored in the apps — a clip longer than one render is chained and stitched by the machine automatically. Script one with `mold run --script` or the `/api/chain-jobs` API.                                                                  | Same: long clips chain automatically, no scene composer.                                                                                                                                                                                                                                                         |
