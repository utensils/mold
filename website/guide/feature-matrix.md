...
| Library          | Unified local/remote grid with host filters, deduplication, batch provenance, History drawer, and desktop file actions                 | Merged saved-host library with batch/source provenance, full-screen image/video, swipe navigation, Use as prompt, and Use as source |
| Models           | Installed/Discover, kind and explicit 18+ NSFW badges, rich detail, host-targeted Pull/Repair, download progress                       | Installed/live union, matching kind/18+ badges and detail, host picker, pull progress, cancel, load/unload/remove                   |
...
| Chains and cloud | Chains authoring/jobs (inside Create) and RunPod provisioning (inside Machines)                                                        | Generation-time video/auto-chain controls only; no Chains or RunPod workspace yet                                                   |
