...
| `MOLD_DISPATCH_MODE` | `v2`                               | Restart-time GPU dispatch owner: `legacy` retains the prior depth-two transport while sharing one binary execution claim with chains, `observe` retains legacy ownership and records request/placement/host-memory-feasible V2 decisions read-only, and `v2` enables authoritative V2 leases. Queue pause gates generation, utility, and admin GPU work in every mode. Invalid values fail startup. |
...
| `MOLD_LTX2_GEMMA_DEVICE`              | `auto`                                  | LTX-2 Gemma 3 12B prompt encoder placement: `auto` uses the GPU leased to the stage when it has more than 24 GB free, otherwise CPU; it never allocates on an unleased sibling GPU. `cpu` forces system RAM (slower, but no VRAM contention); `gpu` pins the assigned GPU and surfaces OOM instead of auto-offloading. The deprecated `MOLD_LTX2_DEBUG_FORCE_CPU_PROMPT_ENCODER=1` is a one-shot-warn alias. Server preflight uses the same resolver as runtime.                   |
