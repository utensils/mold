...
- **`config.toml`** — the hand-editable _bootstrap_ file at
  `$MOLD_HOME/config.toml` (default `~/.mold/config.toml`). Owns paths, ports,
  credentials, and the model-path entries that `mold pull` writes.
  `~/.config/mold/home` is a one-line pointer file that names `$MOLD_HOME`; it
  is not a second config file.
- **`mold.db` (SQLite)** — owns user preferences: the `[expand]` section,
  scheduler timing preferences (`scheduler.*`), gallery and queue retention
  (`gallery.trash_retention_days`, `queue.held_retention_days`),
  `generate.auto_tag_title`, the TUI's `tui.*` preferences,
  ...
  `default_width`, `default_height`, `default_frames`, `default_fps`,
  `scheduler`, `negative_prompt`, `lora`, `lora_scale`). These fields moved to the DB in
  [#265](https://github.com/utensils/mold/issues/265) so GUI writes and
  hand-curated TOML no longer fight over the same file. `default_frames` and
  `default_fps` are DB-owned but written by the generating surfaces:
  `mold config set models.<name>.<field>` accepts only the eight other
  per-model fields, and `umt5_variant` is a registered key name with no
  read/write arm and no DB slot ([#778](https://github.com/utensils/mold/issues/778)) —
  use `MOLD_UMT5_VARIANT`.
...
| `MOLD_ARTIFACT_ATTESTATIONS_DIR`  | `$MOLD_HOME/.artifact-attestations-v1` | Owner-private digest receipts written at acquisition; existing models use local installation identities when receipts are unavailable. Set this to private service state when `MOLD_HOME` is shared or group-writable                                                                                                                                                                |
...
| `MOLD_KEEP_TE_RAM`                    | `auto`                                             | Whether text encoders stay in host RAM between requests instead of being dropped. **Tri-state.** `auto` (unset) measures the host: a park is admitted only when the encoder, the transformer that loads beside it, and a `max(15 % of RAM, 8 GiB)` safety floor all fit in available memory — so a 64 GB desktop streams as it always did and a 1.5 TB host parks and page-locks. `1` parks whenever the encoder alone clears the floor (an operator saying they know the machine), and is also the unchanged opt-in every other family reads: FLUX/SD3's T5, Wan's UMT5. `0` never parks. Disabled on Metal, where the "parked" copy would sit in the pool the encoder already runs from. Quantized GGUF encoders park too — Qwen-Image's Qwen2 since [#1044](https://github.com/utensils/mold/issues/1044), and Flux.2 Klein's and Z-Image's Qwen3 now — with the `QTensor` bytes moving host↔device losslessly rather than being re-read from disk. FLUX.2 [dev] parks the Mistral3 prefix it streams (~35 GB at BF16), filtered to the layers it actually runs, never the whole 36 GB file. |
...
| `MOLD_VAE_TILED`                      | `auto`                                             | Tiled VAE decode for FLUX/FLUX2/SDXL/SD3 and Z-Image on Metal: `auto` retries with tiling on OOM, `force` always tiles, `off` disables tiling. Z-Image completes Metal attempts before accepting pixels                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `MOLD_ATTN`                           | per family: `flash` for video and FLUX, `math` for other images | Attention backend: `math` (hand-rolled SDP) or `flash` (candle-flash-attn v2). Flash needs a `--features cuda,flash-attn` build (the shipped sm89 `h3-cuda` artifact is one) and a CUDA fp16/bf16 tensor. **The default is per family.** Video DiTs (Wan, LTX-2) default to `flash` wherever the kernel is compiled in, because a clip is re-rendered for its content and the math path costs 2.1x. **FLUX.1 and FLUX.2 also default to `flash`** wherever the kernel is compiled in: for those two the math score matrix is the render rather than a rounding error beside the weights (~700 GB of score-matrix traffic per step at 1024²). Every other image family defaults to `math` in every build, so the image an archived seed renders never depends on which artifact you installed. Setting this variable overrides all three. FLUX's dense BF16 transformer arm is the exception on the flux side — it attends through upstream Candle and is unaffected. Reaches FLUX offload/GGUF-bypass, all Flux.2, Z-Image offload, the Wan DiT's self/cross attention, and all three Qwen-Image transformers (BF16, quantized, offloaded) on CUDA/CPU — Qwen keeps candle's fused `sdpa` on Metal; dense BF16 FLUX and Z-Image use upstream Candle's own attention and are unaffected. On Wan, flash is a speed lever rather than a memory one: measured on an RTX 4090 (`wan22-t2v-a14b:q5`, 53f/832x480, same binary, only the backend varying) it renders in 75.3 s against math's 158.4 s — 2.1x — while peak VRAM falls only from 22,250 MiB to 21,354 MiB. Ineligible tensors fall back to math silently; a build without the feature warns once. |
| `MOLD_FLUX2_QMATMUL`                  | `1`                                                | Flux.2 GGUF linear arm on CUDA: `1` (default) keeps candle's quantized fast path, `0`/`off` restores the per-forward dequantization. The default is on — unlike Qwen-Image's and Z-Image's equivalents — because FLUX has rendered correctly through those kernels for as long as it has loaded GGUFs. Set it to `0` if a Flux.2 render comes out black or blank, and say so in an issue. |
| `MOLD_FLUX2_FP8_CACHE`                | budgeted                                           | How a Flux.2 FP8 tier holds its weights. Unset (and any value other than `0`/`1`) measures the card before the first weight lands: if three copies of the checkpoint plus the runtime headroom fit in usable free VRAM, the F8 slab is widened to the working dtype **once at load** and dropped — two bytes per parameter at rest, and no per-forward cast. Otherwise the slab is kept and widened on every forward, which is the historical behaviour and the answer whenever free VRAM cannot be read at all (CPU, Metal, a build without NVML). `1` forces the widened arm, `0` forces the per-forward one. Both arms are the same arithmetic in the same order — the per-tensor scale still rides the matmul output — so this is a residency/latency choice, not a numerical one. |
| `MOLD_FLUX2_FP8_GEMM`                 | off                                                | `1` routes Flux.2 FP8 linears to the fork's native cuBLASLt `(F8E4M3, F8E4M3) -> BF16` GEMM instead of widening the weight. **Unqualified and off by default**: it quantizes the *activation*, which the widen path never does, and nothing here has been compared against a ComfyUI `torch._scaled_mm` parity fixture. It warns once when enabled. CUDA only; ignored everywhere else. |
| `MOLD_FLUX_DEBUG_NONFINITE`           | off                                                | `1` checks every FLUX.1 / Flux.2 denoise step for NaN or infinity and fails the render naming the step. Off by default and a single boolean when off; on, it costs one device synchronization and one reduction per step. A diagnostic, not an engine input — it cannot change a pixel. |
...
| `MOLD_CONV`                           | per family: `cudnn` for video and FLUX, `im2col` for other images | Convolution backend: `im2col` (materialise the column buffer, then one GEMM) or `cudnn` (NVIDIA's kernels). `cudnn` needs a `--features cuda,cudnn` build — every Linux CUDA release artifact is one — and is chosen per convolution inside candle, which stays on im2col for 1x1 kernels and for shapes too small to repay cuDNN's per-call setup, and falls back to im2col if a cuDNN call errors. Explicit per-layer algorithms bypass the automatic size heuristic when cuDNN is enabled. The Hunyuan3D paint material upscaler scopes its own cuDNN default; shape generation retains the image default. `MOLD_CONV` overrides both. **The default is otherwise per family**, on the same argument as `MOLD_ATTN`: video families (Wan, LTX-2) and FLUX.1/FLUX.2 default to `cudnn`, every other image family to `im2col` in every build so an archived still's bytes never depend on which artifact you installed. FLUX's transformer has no convolutions at all, so the flux scope covers VAE encode and decode. Measured on an RTX 4090 (`wan22-t2v-a14b:q5`, 832x480, one steady-state latent frame of the VAE decoder): 845 ms of convolution on im2col against 192 ms on cuDNN, a 4.4x reduction that takes the decode from ~21% of the render to ~10%. The two backends sum in a different order and do not agree bit-for-bit.                                                     |
| `MOLD_HUNYUAN3D_DECODE_CHUNKS`        | `8000` (`32000` on Metal)                          | Query points the Hunyuan3D shape VAE decodes per chunk. Metal's default is measured (M4 Max, octree 256: 32000 takes 19% off the volume decode and yields a byte-identical mesh; 100000 is slower again). The occupancy field is evaluated on `(octree + 1)³` points — about 17 million at the default resolution 256 — so this is what keeps the decode bounded rather than materialising the whole grid at once. Lower it if the decode is the stage that runs out of VRAM; raising it trades memory for slightly fewer kernel launches. Clamped to 256..=1,000,000. Chunk boundaries change the order floating-point sums are accumulated in, so two renders that disagree on this value are not bit-identical and mold treats them as different execution classes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `MOLD_SCHEDULER`                      | —                                                  | SD1.5/SDXL: `ddim` / `euler-ancestral` / `uni-pc` / `edm-dpm-pp-2m` (Playground v2.5 only). Wan solvers are selected with `--sample-solver` (`unipc` / `euler` / `dpm++`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `MOLD_LTX2_INT8`                      | `native`                                           | How LTX-2.5 `int8-conv` transformer linears execute on CUDA. `native` (default) keeps the checkpoint packed on the device (one byte per weight plus one F32 scale per output row) and runs ComfyUI's own W8A8 order — rotate the activation, dynamically quantize it per row, INT8×INT8→INT32 cuBLASLt GEMM, both scales applied in F32, bias added after the GEMM — the same execution comfy-kitchen's `int8_linear` performs for these exact checkpoints. `dequant` is the W8A16 escape hatch for A/B comparison: each linear is widened to the activation dtype per forward through the fused ConvRot kernel (one transient dense linear, never a whole block) and multiplied like an ordinary weight. Metal and CPU ignore it — they keep the widening arm regardless. The active arm is logged once as `ltx2 int8 arm=…` and the two settings are registered as distinct engine-shaping classes, so fingerprints and learned timings never mix them.                                                                                                                                                                                                                                                                                                           |
| `MOLD_LTX2_QMATMUL`                   | `0`                                                | Quantized (GGUF) LTX-2.5 transformer linears on CUDA. **Experimental, off by default**: `1` (or `true`/`on`/`yes`) keeps the block linears quantized through candle's MMQ/MMVQ kernels — the same kernels that return 100% NaN for Qwen-Image and render Z-Image solid black ([#1048](https://github.com/utensils/mold/issues/1048)), so the default dequantizes each linear per forward at the LTX compute dtype instead. Metal and CPU ignore it: Metal keeps `QMatMul` and CPU always dequantizes. The active arm is logged once as `ltx2 linear kind=qmatmul` / `ltx2 linear kind=dequant`, and the flag is registered as an engine-shaping variable so fingerprints and learned timings never mix the two arms.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `MOLD_WAN_STEP_CACHE`                 | `off`                                              | Wan first-block residual reuse ([#801](https://github.com/utensils/mold/issues/801)) for qualified non-distilled quality tiers: `off`, `auto` (relative-L1 threshold 0.10), or an explicit positive threshold. Block 0 runs every step; when its residual moves less than the threshold, blocks 1..N are skipped and their previous contribution is replayed. Approximate reuse is explicit because a Metal 1.3B qualification produced saturated fields with the cache on and a coherent scene with it off. It is refused with a message for Wan 1.3B and dense Wan 2.1 14B (including explicit thresholds), when a distill adapter is active, or when the schedule is under 12 steps; Wan 2.2 A14B remains qualified. Changing the threshold changes the output: a cached run is a different sample, not the same frames faster. `off` is bit-identical to denoising every block. The 0.10 threshold remains available for workloads where it has been visually qualified; it measured 1.85x on an RTX 4090 (`wan22-t2v-a14b:q8`, 33f/832x480, 20 steps).                                                                                                                                                                                                         |
...
| `MOLD_H3_TURBO_TIER`                  | unset                                              | **Capture-scope UAT override, honored only by `h3-private-uat` builds** — see `MOLD_H3_TURBO_ADAPTER`. Which reviewed Turbo tier the file at `MOLD_H3_TURBO_ADAPTER` must be. Accepts the short alias `fl2v-8step` (9 grid points, 8 transformer evaluations, trained at 544p, base video shift), `fl2v-4step-768p` (5 grid points, video shift 6), `fl2v-4step-768p-v1.1` (5 grid points, video shift 6), `fl2v-8step-768p` (9 grid points, video shift 6), `ref2v-4step` (5 grid points), `fl2v-4step-768p-r21` (5 grid points, video shift 6), `fl2v-8step-r21` (9 grid points, video shift 12), or `ref2v-4step-r21` (5 grid points, video shift 12), or the corresponding full stable id such as `minimax-h3.turbo-lora.fl2v-8step-v1.0.comfyui-bf16.v1`. Matching is case-insensitive. An unreviewed value is rejected and names the accepted set. Note mold counts terminal-inclusive grid points, so a "Turbo 8-step" checkpoint is 9 mold steps and exactly 8 forwards. Registered as an engine-shaping variable.                                                                                                                                                                                                                                          |
| `MOLD_H3_CONDITIONER_CACHE`           | `512` (MiB)                                        | Byte budget for the in-process cache of MiniMax H3's Qwen conditioner output ([#814](https://github.com/utensils/mold/issues/814)). A repeated prompt against the same first-frame/reference set, conditioner route, and device serves the prior render's conditioning tensor instead of reloading and re-encoding the 15.7 GB checkpoint — the output is bit-identical, disclosed as `prompt conditioning [cache hit]`. `off`, `0`, `false`, `no`, or `disabled` (case-insensitive) turns the cache off; `<MiB>` sets the resident-bytes budget, minimum 1 and clamped to 65536 (64 GiB) so a typo cannot produce a budget no entry can reach; an unparseable value keeps the default and warns; unset defaults to 512 MiB. Read straight from the environment like `MOLD_LTX2_KEEP_SESSION`, and deliberately **not** registered as an engine-shaping variable — a hit restores the identical tensor a miss would have produced, so it moves wall clock and host residency only, never weights, device choice, or numerics. Inert in `h3-private-uat` capture-scope builds so a capture run always exercises the conditioner.                                                                                                                                     |
...
| `MOLD_QUEUE_SIZE`                   | `200`               | Jobs hydrated into the runtime dispatch window; overflow returns HTTP 503 `QUEUE_FULL` with `Retry-After`. The durable SQLite backlog is **not** bounded by this value — it holds every admitted job until it runs, is cancelled, or is swept                                                                                                                                                                                                                                                              |
...
| `MOLD_DESTINATION_API_KEY`          | —                   | Credential for the DESTINATION host when handing a queued job to another machine (`mold queue handoff --to`); independent of the source `MOLD_API_KEY`                                                                                                                                                                                                                                                                                                                                                     |
...
There are two PuLID bundles — `pulid-flux` for FLUX.1 checkpoints and
`pulid-sdxl` for SDXL ones — and they share the one antelopev2 acceptance, so
recording it for either unblocks both. See [Identity](/guide/identity).
...
(override with `MOLD_DB_PATH`). All three surfaces — the CLI's local
generation path, the TUI, and the HTTP server — write a row per saved file:
prompt, negative prompt, model, seed, steps, guidance, dimensions, LoRA,
scheduler, the file's mtime/size, the generation duration, and a `source`
column (`server` / `cli` / `tui` / `backfill` / `unknown`).
...
A durable generation that cannot proceed is **held** rather than failed: it
stays listed in `GET /api/queue`, keeps its staged request media, and waits for
...
Age is measured from when the row was **held**, not when it was admitted, so
work that waited a week in the queue and held yesterday still gets its full
window. The server sweeps hourly and reads the key fresh on every pass;
`POST /api/queue/held/sweep` runs one pass on demand.

Purging a held row releases the encrypted request media that row pinned, and
settles its batch child as `failed` so a reconnecting client still sees a
