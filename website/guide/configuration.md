...
| `MOLD_KEEP_TE_RAM`                    | auto (CUDA) / off                       | `1` forces text encoders to park on CPU host RAM between requests; any other value forces the drop-and-reload. Unset, Qwen-Image now defaults **on** for CUDA builds when `/proc/meminfo` `MemAvailable` covers the encoder's own bytes plus an 8 GiB margin — that is what removes the measured 35.1 s per-request encoder reload. Other families remain opt-in. Covers quantized GGUF encoders since [#1044](https://github.com/utensils/mold/issues/1044): their `QTensor` bytes move host↔device losslessly rather than being re-read from disk. Disabled on Metal (unified memory). Includes Wan's UMT5-XXL, where the saving is largest: the parked copy keeps the checkpoint's F16, so a second consecutive render skips an 11.4 GB disk read for ~11.4 GB of host RAM.                                                                                                                                            |
