...
| `MOLD_QWEN_QMATMUL`                   | `1`                                     | Quantized (GGUF) Qwen-Image linears on CUDA ([#1045](https://github.com/utensils/mold/issues/1045)). The default keeps the checkpoint quantized and feeds candle's MMQ/MMVQ kernels directly; `0` restores the previous arm, which dequantized every weight to BF16 on every forward. Weights the kernels cannot take (an `IQ*` or float-stored tensor) and CPU-staged weights keep the dequant arm regardless. Metal and CPU ignore it. Changes numerics slightly (int8 MMQ vs a BF16 GEMM over dequantized weights), so it is registered as an engine-shaping variable and never shares a scheduler fingerprint or learned estimate with the other setting.                                                                                                                                                                                                                                                                                        |
| `MOLD_QWEN_FP8_CACHE`                 | off                                     | `1` keeps the widened BF16 copy of an FP8 Qwen-Image checkpoint's weights instead of re-casting them every forward ([#1045](https://github.com/utensils/mold/issues/1045)). Roughly doubles the transformer's resident VRAM — an FP8 build is normally chosen precisely because that headroom is missing, so this is opt-in and only worth it on a card with room to spare.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
