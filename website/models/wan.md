...
Every parked block is a host round-trip on the critical path of every step, so the 81-frame render was paying full weight streaming for a shortfall that was largely an artifact of pricing the wrong backend. That is the opposite of the usual expectation, and it is why longer clips are not unlocked by switching backends: at 81 frames the estimate is ~27.7 GB against ~24.8 GB usable, and flash's measured per-token saving extrapolates to about 1.3 GB — not the ~3 GB that would be needed. Reaching 81 frames on a 24 GB card needs partial block offload, which is now wired for this family (see above). Note also that every shipped Linux CUDA artifact compiles `flash-attn` — `mold` (sm89, through `h3-cuda`), `mold-sm86`, `mold-sm100` and `mold-sm120`, plus the matching desktop packages — so video families reach flash on all of them by default. Only a self-built `--features cuda` binary without `flash-attn`, and Metal builds, stay on math.
