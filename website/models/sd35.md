...
Every SD 3.5 variant is `[gated]`: the shared VAE, CLIP-L, CLIP-G, and T5-XXL
files all come from `stabilityai/stable-diffusion-3.5-large`. Accept the
Stability AI Community License on HuggingFace and set `HF_TOKEN` before
`mold pull` — see [HuggingFace Auth](/models/#huggingface-auth).

`--offload` applies only to a BF16/FP SD 3.5 transformer you supply yourself;
every shipped variant is GGUF, for which block-level offload is refused (as is
offload combined with a LoRA).
