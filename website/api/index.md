...
| `POST`   | `/api/mesh-workflows`                            | key           | Create a validated durable text-to-3D or supplied-mesh texture workflow                                                                                                                                                                                                                                      |
| `GET`    | `/api/mesh-workflows`                            | key           | List durable 3-D workflows                                                                                                                                                                                                                                                                                   |
| `GET`    | `/api/mesh-workflows/:id`                        | key           | Read one workflow and its stage checkpoints                                                                                                                                                                                                                                                                  |
| `GET`    | `/api/mesh-workflows/:id/events`                 | key           | Follow workflow snapshots over SSE                                                                                                                                                                                                                                                                           |
| `POST`   | `/api/mesh-workflows/:id/resume`                 | key           | Resume a parked or failed workflow                                                                                                                                                                                                                                                                           |
| `POST`   | `/api/mesh-workflows/:id/cancel`                 | key           | Cancel the workflow and every attached child                                                                                                                                                                                                                                                                 |
| `DELETE` | `/api/mesh-workflows/:id`                        | key           | Delete a settled workflow and release its retained inputs/stage artifacts; the published Library print remains                                                                                                                                                                                               |
...
| `edit_images`, `reference_weight`                                                                       | ordered reference images. What they MEAN is the recipe's answer, in `capabilities.reference_images.source_relation`: `replaces` (Qwen-Image-Edit, whose FIRST image is the edit target, and FLUX.2 [dev]) reads no `source_image` at all; `exclusive` (FLUX.2 [klein]) renders from a source image OR references, never both; `combines` (SD 1.5 and SDXL, IP-Adapter image prompting) reads BOTH in one pass and refuses none of the img2img fields, and is the only relation exempt from `batch_size = 1`. `reference_weight` is the injection strength for a recipe that advertises `capabilities.reference_images.weight` — `0.0..=2.0`, absent = `1.0`, and exactly `0.0` is inert: nothing is downloaded, loaded or encoded, and the render is pixel-identical to one carrying no reference. A recipe with no `weight` block refuses the field rather than ignoring it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `enable_audio`, `audio_file`, `audio_file_path`                                                         | LTX-2 synchronized audio toggle and audio-to-video input. `enable_audio` defaults to **on** wherever the recipe delivers audio, so an omitted field renders with sound and a caller wanting silence sends an explicit `false`. Path input is server-local and requires configured `media_roots` / `MOLD_MEDIA_ROOTS`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
