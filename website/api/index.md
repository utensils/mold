...
| `POST`   | `/api/pairing/sessions`                       | Mint an authenticated, one-use, two-minute mobile pairing ticket                                                                                                                                                                                                    |
...
| `GET`    | `/api/gallery/thumbnail/:name`                | Fetch a cached thumbnail; `?size=256\|512` and `?fmt=png\|jpeg` select a rendition (default 256 px PNG, unchanged); the reply names it in `x-mold-thumbnail-rendition` (`512-jpg`) so clients can tell an older server, which ignores the query, from a small print |
  ...
  "references": [
    {
      ...
      "provenance": { "name": "hero.png", "sha256": "<sha256>" },
      "mime_type": "image/png",
      "width": 2048,
      "height": 2048
    }
