...
| `id_image`, `id_image_name`, `id_weight`, `id_start_step`    | Face-identity conditioning (PuLID-FLUX). `id_image` is base64 PNG/JPEG bytes, bounds-checked from its header alone (≤ 16 MiB encoded, ≤ 8192 px per axis, ≤ 32 MP) before any decode. `id_weight` is `0.0..=3.0` (absent = `1.0`); `id_start_step` is the first identity-conditioned denoise step and must be `< steps` (absent = `0`). Milestone 1 accepts identity only on the qualified checkpoints `flux-dev:q4` / `flux-dev:q8` (bare `flux-dev` resolves to `:q8`; the legacy `flux-dev-q4` resolves too) and refuses it combined with a LoRA or with an img2img `source_image`. Any of the three companion fields without `id_image` is an error, not an ignored field. A server that cannot execute identity conditioning refuses any request carrying an identity field rather than rendering without the face, with two distinct messages: a build without the `pulid` feature says it was built without PuLID support (a differently compiled binary is needed), and a build that links `pulid` while the runtime adapter is still pending says identity conditioning is not available in this build yet (a newer one is needed). No release advertises the capability until that adapter lands, so check `/api/models[].supports_identity` before offering the control and never infer support from the build feature. An identity request materializes the `pulid-flux` asset bundle itself: `POST /api/generate/placement-preview` reports anything missing under `pending_downloads` (kinds `identity_adapter`, `identity_vision_encoder`, `face_detector`, `face_recognizer`) without fetching it, and admission downloads it after the InsightFace license gate. `id_weight` `0` is inert — it plans and downloads nothing. |
