...
record_media() {
  ...
  local generator_commit=contract_test
  if [[ "$contract_test" != 1 ]]; then
    local generator_version
    generator_version="$(jq -er '.metadata_json | fromjson | .version' <<<"$row")"
    if [[ "$generator_version" =~ \(([0-9a-f]{7,40})[[:space:]] ]]; then
      generator_commit="${BASH_REMATCH[1]}"
    else
      fail "$label generation metadata has no source commit: $generator_version"
    fi
    [[ "${source_commit:0:${#generator_commit}}" == "$generator_commit" ]] \
      || fail "$label was generated by $generator_commit, not source commit $source_commit"
  fi
  media="$(jq -c \
    --arg label "$label" --arg path "$path" --arg sha "$(file_sha256 "$path")" \
    --arg prompt "$prompt" --arg generator_commit "$generator_commit" \
    --argjson seed "$seed" --argjson audio "$expects_audio" \
    --argjson generation "$row" --slurpfile probe "$probe" \
    '. + [{label: $label, path: $path, sha256: $sha, prompt: $prompt, seed: $seed,
      model: "ltx-2.5-22b-distilled:int8-conv", generator_commit: $generator_commit, settings: {
