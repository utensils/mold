...
(a.output/'completed.json').write_text(json.dumps(dict(revision=revision,size=a.size,views=views,torch=torch.__version__,mesh_sha256=hashlib.sha256(a.mesh.read_bytes()).hexdigest(),records=records,diagnostic_bake_input=str(a.diagnostic_bake_input) if a.diagnostic_bake_input else None,opencv=cv2.__version__,native_sha256={str(p.relative_to(a.native)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(a.native.rglob('*.so'))},source_sha256={str(p.relative_to(a.reference)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [source,a.reference/'hy3dpaint/DifferentiableRenderer/MeshRender.py',a.reference/'hy3dpaint/DifferentiableRenderer/mesh_inpaint_processor.cpp']}),indent=2)+'\n')
