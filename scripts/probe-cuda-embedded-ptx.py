...
MANIFEST_SCHEMA = "mold.cuda.ptx-manifest.v1"
...
def extract_elf_section(binary: Path, section: str, output: Path) -> bytes:
    ...
    if result.returncode != 0 or not output.is_file():
        fail(
            f"ELF section {section} is missing or unreadable; "
            "release PTX must be sealed by seal-cuda-ptx-manifest.py"
        )
...
def sealed_ptx_modules(
    ...
    with tempfile.TemporaryDirectory(
        ...
        temp_dir = Path(temp)
        manifest_bytes = extract_elf_section(
    ...
    try:
        ...
        fail(f"invalid {MANIFEST_SECTION} JSON: {error}")
    ...
    if not isinstance(raw_modules, list) or not raw_modules:
        fail(f"{MANIFEST_SECTION} must name at least one generated PTX module")
    ...
    for index, raw_module in enumerate(raw_modules):
        if not isinstance(raw_module, dict) or set(raw_module) != required_keys:
            fail(f"{MANIFEST_SECTION} module {index} has an invalid shape")
        source_name = raw_module["source_name"]
        offset = raw_module["rodata_offset"]
        length = raw_module["byte_length"]
        expected_sha = raw_module["sha256"]
        if (
            not isinstance(source_name, str)
            or Path(source_name).name != source_name
            or not source_name.endswith(".ptx")
        ):
            fail(f"{MANIFEST_SECTION} module {index} has an invalid source name")
        if (
            ...
            fail(f"{MANIFEST_SECTION} module {index} has an invalid byte range")
        if not isinstance(expected_sha, str) or not re.fullmatch(
            r"[0-9a-f]{64}", expected_sha
        ):
            fail(f"{MANIFEST_SECTION} module {index} has an invalid SHA-256")
        end = offset + length
        if end > len(rodata):
            fail(f"{MANIFEST_SECTION} module {index} is outside ELF .rodata")
        module = rodata[offset:end]
