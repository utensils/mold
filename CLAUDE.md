...
- **A print knows which 3-D workflow made it.** Every stage of a durable mesh
  ...
  desktop LIBRARY: `studio/lib/meshWorkflowGroup.ts` collapses a run into one
  tile led by its `final_glb`, indexed once per data change beside
  `organizationIndex` and applied in `filtered` — the getter the grid renders,
  not `basePrints`, which feeds only the sidebar count and the chips. TRASH IS
  NOT COLLAPSED and the index is live-only: every trashed print carries its own
  purge countdown and its own Restore, and hiding one behind a lead would let
  retention purge something nobody was shown. Web and the phone still list
