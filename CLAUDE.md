...
The web IA uses Create (`/create`, also the `/` default), Library (`/library`), Models (`/models`), Machines (`/machines` + `/machines/:id`), and Settings (`/settings`). Retired `/generate` and `/catalog` paths and all unmatched URLs render Page Not Found. Web accepts only the current storage and server wire shapes; an incompatible remote surfaces an actionable upgrade error. Sub-640px swaps the nav pills for a hamburger + bottom nav sheet. The shell owns a ⌘K palette, a downloads center, and app-frame toasts/dialogs (never `window.alert/confirm/prompt`). Per-surface Advanced policy (spec §06 surface split): web at ≥ tablet width is the power surface — the Advanced sections render inline in the Create controls region (accordions collapsed, count badge + Reset kept), never behind a drawer; mobile web keeps the sheet, and the desktop app + iPhone deliberately keep the single Advanced drawer/sheet entry. Web Machines is browser-side multi-host: a localStorage host registry (`mold.web.hosts.v1`, origin always primary, instance-UUID dedupe) with per-host `x-api-key` headers — keys never in URLs. Its Local network flow is server-assisted: only when the primary reports `discovery.can_browse` does the browser request `GET /api/discovery/peers`, filter remembered instance UUIDs, then test-connect directly to the peer before saving; older servers are never probed. The results reflect the primary server's LAN and do not proxy traffic. Theme families persist at `mold.web.theme.v1` and apply before first paint via the inline `index.html` script.
