...
- **An auto-chained one-shot is not an authored sequence.** `ChainRequest.ephemeral` (additive, absent means authored) is the distinction, set on the ONE creation path: `mold run --frames 200` renders a long video as a chain because the model cannot do it in one pass, so its job is absent from `GET /api/chain-jobs`, emits no `chain_job_queued` (History ▸ Sequences and "Now developing" both read that), is swept after finalization, refuses resume, and — the half a user sees — publishes a print with `chain_job_id: None`, so Reuse settings restores a one-shot instead of opening the clip rail for a job `sequenceReuse` cannot load. It DOES publish its print, with full per-clip provenance: the deleted shim returned bytes inline and published nothing, which is not an option once the client fetches from the gallery. Stage seeds are recorded either way — they describe how the pixels were made, not who authored the split.
