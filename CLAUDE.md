...
- **LTX-2 runtime-asset bundling is mixed.** Catalog checkpoints may be combined or transformer-only; resolve `ltx2-vae` for v2 and `ltx2.3-vae` for v2.3. Diffusion-only LTX-2.3 exports also require `ltx2.3-text-projection`. Preserve those concrete paths in every chain-stage plan. ConvRot W4A4 checkpoints use the dedicated `ltx2/convrot.rs` compatibility backend. A Gemma prompt-encoder OOM may move only Gemma to CPU; preserve the transformer and video VAE CUDA device through the retry.
