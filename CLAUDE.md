...
**Web gallery UI** (separate from the docs site): Vue 3 + Vite 7 + Tailwind CSS v4.2 SPA in `web/`. Build with `cd web && bun install && bun run build`. Dev server: `bun run dev` (proxies `/api` + `/health` to `http://localhost:7680`; override with `MOLD_API_ORIGIN`). The mold server locates the built SPA via `MOLD_WEB_DIR`, `$XDG_DATA_HOME/mold/web`, `~/.mold/web`, `<binary dir>/web`, or `./web/dist` (first hit wins) and serves it as the axum fallback — `/api/*`, `/health`, and `/metrics` take precedence because they are matched first. When no bundle is found, a small inline placeholder points users at `/api/docs`. See `crates/mold-server/src/web_ui.rs` for the resolver and `web/README.md` for the frontend stack.
