...
**iPhone app** (`apps/mobile/src-tauri`, frontend entry in `desktop/src/mobile/`): a standalone thin Tauri crate, excluded from the root workspace so the desktop GPU/server dependency tree is never cross-compiled for iOS. It is remote-only and initially owns Generate, Gallery, and Hosts. Reuse the desktop's `src/lib/api/types.ts`, explicit-target HTTP/SSE helpers, capability matrix, and `generateForm.ts`; do not import stores that assume a local primary. Host metadata lives in WebView storage but API keys must remain in the native iOS Keychain. Host entry accepts LAN/IP/DNS/HTTPS and Tailscale MagicDNS names, while native DNS-SD discovers `_mold._tcp` under the iOS local-network permission. The UI is iPhone-first with safe-area layout and 44pt controls; iPad is a responsive secondary target. Devshell: `ios-dev` / `ios-run` / `ios-check` / `ios-build`; CI: `.github/workflows/ios.yml`; internal nightlies: `.github/workflows/testflight-ios.yml`. The mobile Vite build must rename its separate source entry to `dist-mobile/index.html`, and `scripts/tests/ios-release-assets.sh` is the release guard for that embedded entry plus the opaque Apple icon catalog; regenerate the catalog with `scripts/generate-ios-icons.sh`. The mobile crate, Tauri config, and generated Apple project versions must track the workspace release.
