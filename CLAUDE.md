...
The web IA mirrors the five workspaces: Gallery (`/`), Create (`/create`), Models (`/models`), Machines (`/machines` + `/machines/:id`), Settings (`/settings`, reached from the avatar/nav sheet); `/generate` and `/catalog` are permanent redirects. Sub-640px swaps the nav pills for a hamburger + bottom nav sheet. The shell owns a ⌘K palette, a downloads center, and app-frame toasts/dialogs (never `window.alert/confirm/prompt`). Web Machines is browser-side multi-host: a localStorage host registry (`mold.web.hosts.v1`, origin always primary, instance-UUID dedupe) with per-host `x-api-key` headers — keys never in URLs. Theme families persist at `mold.web.theme.v1` and apply before first paint via the inline `index.html` script; imports from the shared kit use the `@ui` alias (repo-root `ui/`, included in the Nix build via fileset union — keep `flake.nix` in sync if `ui/` moves).
