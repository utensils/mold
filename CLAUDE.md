...
- **A text-only tier's one-shot is refused above its clip size, at every door.** `mold_core::chain::text_only_wan_auto_chain_refusal` is the single authority and the single sentence: a wan tier whose `source_image` contract is `Unsupported` hands nothing across a clip boundary, so an auto-chain re-derives the scene from the same prompt and seed at every stage and delivers the same clip repeated with a visible reset. The CLI router calls it, `studio/lib/chainRouting.ts` mirrors it byte-for-byte, and `POST /api/chain-jobs` answers 422 from it for an `ephemeral` request — `tests/fixtures/wan/surface-parity-v1.json` pins the template and both the Rust and the vitest side read it. Only the split mold performs on the user's behalf is refused; an AUTHORED sequence still composes repeated clips, because there that is what the author asked for.
