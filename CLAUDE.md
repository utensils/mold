...
**Metadata DB at `MOLD_HOME/mold.db`** (override with `MOLD_DB_PATH`, disable with `MOLD_DB_DISABLE=1`). The `mold-db` crate (rusqlite + bundled SQLite, WAL mode) owns four tables (current schema: `SCHEMA_VERSION = 5`):
...
| `settings` (v3) | Typed KV for TUI state and user-facing preferences — `tui.theme`, `tui.last_model`, `tui.last_prompt`, `tui.negative_collapsed`, `tui.migrated_from_json` sentinel, `expand.*`, `generate.*` (room for future sections). | `mold_db::Settings` |
| `model_prefs` (v4) | One row per resolved model tag with the generation params the TUI last used for that model (width, height, steps, guidance, scheduler, seed_mode, batch, format, lora_path, lora_scale, expand, offload, strength, control_scale, frames, fps, last_prompt, last_negative). Keyed on `manifest::resolve_model_name(name)` so `flux-dev` and `flux-dev:q4` collapse to one row. | `mold_db::ModelPrefs` |
