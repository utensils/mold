...
**Desktop prepared-expansion invariant:** Batch 1 keeps quick Expand/undo but freezes its resolved route through the next Generate and becomes stale if its prompt/model/family/host inputs change. Batch N greater than 1 must resolve the Generate route once, request exactly N non-empty prompts from that target, preserve the reviewed set across upstream changes as specifically named stale work, and submit one edited prompt per sibling through the stored route with the source prompt as provenance. Prepared siblings carry additive `batch_id`, one-based `batch_index`, and `batch_count` through the request, complete event, saved metadata, and Gallery. Never silently resize, reroute, fall back, or erase prepared work. Missing-model pulls and pull-resume use the same host and retain the ordered prompt list. Generate must use `useDownloadsStore` as the only expansion-pull progress authority, associate the returned job ID with that frozen host/model (with exact-model snapshot compatibility for older timing), and keep Connecting, Starting, Queued, Pulling, Ready, failed, and cancelled recovery inline for both Batch 1 and prepared Batch N. This is desktop-only; do not add iPhone parity without a separate product decision.
