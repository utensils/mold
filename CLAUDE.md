...
├── mold-catalog/     Live HF + Civitai model-discovery proxy (5-min in-proc cache). Depended on by mold-cli, mold-server, and mold-inference — mold-discord MUST NOT depend on it.
...
- **Releases are automated by release-plz.** Never bump versions or edit `CHANGELOG.md` `[Unreleased]` by hand — add a `changelog.d/<slug>.md` fragment. Merging the release PR ships the release. crates.io distribution is retired; use GitHub artifacts, Nix, Docker, AUR, or source builds. Details: `.claude/rules/release-ci.md`.
