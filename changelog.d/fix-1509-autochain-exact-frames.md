- **`--frames N` past one clip now renders N frames, not a whole extra
  ...
  exact-fitted to the requested total on every surface, and a total the
  frame lattice cannot land on exactly is disclosed before rendering
  ([#1509](https://github.com/utensils/mold/issues/1509)).
