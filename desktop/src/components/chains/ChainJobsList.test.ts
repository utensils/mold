...
describe("ChainJobsList clearing", () => {
  it("offers Clear inactive only when inactive jobs exist and clears them", async () => {
    const chains = useChainJobsStore();
    chains.jobs = [job("a", "completed"), job("b", "running"), job("c", "failed")] as never;
    const clear = vi.spyOn(chains, "clearInactive").mockResolvedValue({ cleared: 2, failed: 0 });

    const wrapper = mount(ChainJobsList);
    await wrapper.get("[data-test='clear-inactive']").trigger("click");
    await flushPromises();
    expect(clear).toHaveBeenCalledOnce();
  });

  it("hides Clear inactive while only active jobs remain", () => {
    const chains = useChainJobsStore();
    chains.jobs = [job("a", "running"), job("b", "queued")] as never;

    const wrapper = mount(ChainJobsList);
    expect(wrapper.find("[data-test='clear-inactive']").exists()).toBe(false);
  });
