...
<template>
  ...
  <img
    ...
    draggable="false"
  />
  <div v-else-if="failed" class="flex h-full w-full items-center justify-center bg-bg">
    <span class="font-mono text-micro text-fg-dim whitespace-nowrap">UNREADABLE</span>
  </div>
  <div v-else class="media-placeholder ms-shimmer h-full w-full" aria-hidden="true">
...
.media-placeholder {
  display: grid;
  place-items: center;
  background-color: color-mix(in srgb, var(--mold-bg) 82%, var(--mold-blue) 18%);
  background-image:
    radial-gradient(
      circle at 28% 24%,
