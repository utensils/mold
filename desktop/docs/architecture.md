...
```jsonc
{
  ...
  "bundle": {
    ...
    "createUpdaterArtifacts": false, // flip on when updater ships
  },
}
```
...
- **IPC commands:** `get_connection() -> {base_url, api_key, mode}`, `start_local_engine`, `stop_local_engine`, `set_remote_host{url, api_key}`, `test_remote_host`, `engine_status` (thread alive, port, models_dir), `pick_files{kind}` + `read_file_b64{path}` (feeding `source_image`/`edit_images`/`mask_image`/`control_image`/keyframes/audio as base64 into `GenerateRequest`), `save_bytes_as{path}` (export), `reveal_in_finder{path}`, `app_settings_get/set` (window prefs, last mode — stored in a small `settings.json` under `app_data_dir`, _not_ in mold's config.toml, which stays engine-owned).
...
- **CI:** new `.github/workflows/desktop.yml`, triggered on `push: branches: [experiment/desktop]` and `pull_request: paths: [desktop/**]`. Jobs: `desktop-frontend` (ubuntu: bun install → fmt:check → vue-tsc → vitest), `desktop-rust` (**macos-14**, Apple Silicon: cargo fmt/clippy `-D warnings`/test on the desktop manifest, then `cargo tauri build --bundles app` _without_ metal generation smoke — bundling proof only). Existing CI is untouched (workspace exclude guarantees it).

## 7. Branch & delivery plan

**Branch:** `experiment/desktop` off `main` (long-running; rebase weekly onto main — contact surface is ~3 files so conflicts stay trivial).
...
- **M6 — Polish & ship-readiness:** settings surface (config.toml + DB settings via existing endpoints/expand config), notifications, window-state, single-instance, drag-drop images from Finder, keyboard shortcuts, empty/error states, `mold-desktop` Nix package + signing flow + verify-bundle, docs page.
