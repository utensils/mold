...
**Shutdown:** the embedded handle is owned separately from host connections; app exit or an explicit local-engine restart POSTs `/api/shutdown` and joins the thread within `server::ENGINE_STOP_BUDGET` (5 s; changing Mold home waits twice that). The engine's HTTP drain is bounded for this host (`mold_server::bound_http_drain(server::HTTP_DRAIN_GRACE)`, set on the engine thread before `run_server`): graceful shutdown waits for every in-flight request, and the webview is a client the app does not control — a paused video stops draining its socket — so in-flight requests get the grace and are then given up on, and the runtime drop that follows `run_server` closes them. The standalone `mold serve` keeps its unbounded drain behind its own process-ending deadline. User-run external servers are never shut down by the app.
...
- **Connection budget (HTTP/1.1, ~6 per host):** a print holds no stream at all — it is admitted through one `POST /api/generation-batches` and reconciled over the shared `/api/events` stream — so the budget is spent by auto-chained long videos alone. Each holds a POST-SSE stream for its whole run, and downloads + resources + `/api/events` each hold one more; those streams stay capped at **two concurrent** (`runWithConcurrency` in `stores/generation.ts`). Worst case: 2 chain streams + downloads + resources + events = 5 < 6.
...
- **Frontend:** vitest 4 + `@vue/test-utils`, `happy-dom`. Priority coverage: capability-matrix-driven form logic (ported `generateCapabilities` + `GenerateForm` enable/disable per family), SSE reducer (progress event stream → job card states, including dropped-stream reconciliation via `/api/queue`), updater startup/banner/manual-check/install/error states, and api client (mocked `fetch`, header injection, error envelope parsing). Tauri IPC mocked with `@tauri-apps/api/mocks`.
