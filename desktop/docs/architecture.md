...
**Plugins (all `2.x`):** `dialog` (source image/mask/keyframe/TOML pickers, save-as export), `opener` (reveal in Finder, open URLs), `notification` (generation/chain/pull complete while unfocused), `single-instance` (one engine, one GPU owner), `window-state` (restore geometry), `clipboard-manager` (copy image/prompt), `process` (relaunch), and `updater` (manifest parsing, download, and mandatory Minisign verification; Mold-owned Rust commands retain installation policy). Native webview zoom supplies persistent whole-app scaling. **Deferred:** deep-link (no URL scheme use case yet), `shell` (not needed — no sidecars), `fs` (not needed — file reads happen in Rust commands after dialog returns paths).

**Updater channels and transaction:** the app-local settings store persists `stable` (default) or `nightly`. Startup performs a best-effort check only; menu/Settings checks do the same, and the trusted Rust `Update` remains pending until the user explicitly chooses **Update and restart**. Stable reads public `mold-desktop-stable.json` from the latest tagged release. Nightly reads public `mold-desktop-nightly.json` from the rolling `latest` prerelease, built only for desktop-relevant `main` commits after both desktop CI jobs pass. Default SemVer comparison is intentional: selecting Stable from a newer Nightly waits for a newer stable release instead of silently downgrading data or code.

Tauri verifies the downloaded updater archive before Mold changes the app; the request also has a 15-minute hard timeout. Mold reads the verified archive's `Info.plist` without unpacking it and requires `com.utensils.mold` plus the exact manifest version, then repeats that identity check against the installed bundle. It writes and fsyncs an atomic transaction record under `app_data_dir/updater`, copies the current `.app` there, and launches that copied binary in supervisor mode without a webview. After installation, the old process exits; the supervisor launches the candidate with a one-time health token and waits 60 seconds for preferences, connection startup, window display, and the mounted frontend to confirm health. That marker starts a 10-second process probation; process exit wins even if it lands in the same poll. Install failure, launch failure, early exit, or timeout restores the backup (using an authorized macOS fallback when required) and relaunches the old version. `Installing`/unsupervised `Installed` records reconcile against the running bundle after a reboot and either resume supervision, commit an intact old app, or restore the backup. A failed rollback is terminal and retains the backup path for manual recovery. Cleanup removes and syncs the transaction marker before deleting the backup, and happens only after probation. Running from a DMG or App Translocation is rejected before backup/staging.
...
- **IPC commands:** `get_connection() -> {base_url, api_key, mode}`, `start_local_engine`, `stop_local_engine`, `set_remote_host{url, api_key}`, `test_remote_host`, `engine_status` (thread alive, port, models_dir), `pick_files{kind}` + `read_file_b64{path}` (feeding `source_image`/`edit_images`/`mask_image`/`control_image`/keyframes/audio as base64 into `GenerateRequest`), `save_bytes_as{path}` (export), `reveal_in_finder{path}`, local-gallery list/delete/read commands, `clipboard_write_image` (decode PNG/JPEG/GIF/WebP to RGBA before native clipboard write), RunPod credential/account/inventory/pod/network-volume commands, `check_for_updates` / `install_pending_update` / `confirm_update_healthy` / `take_update_recovery`, and `app_settings_get/set` (window prefs, RunPod selections, update channel, UI scale, last mode — stored in a small `settings.json` under `app_data_dir`, _not_ in mold's config.toml, which stays engine-owned).
...
- **Rust (`desktop/src-tauri`):** unit tests for `settings.rs` (round-trip, migration), `server.rs` port allocation + API-key generation, connection-state machine (local/remote/off transitions), updater endpoint allowlisting, bounded requests, archive/bundle identity binding, interrupted-transaction classification, health probation, bundle swap restoration, unsafe install-location rejection, and supervisor rollback. One `#[tokio::test]` integration test boots the embedded server on an ephemeral port **without GPU features** (CPU build in CI) and asserts `/health`, `/api/capabilities`, and auth rejection without `X-Api-Key`. Feature-gate metal so `cargo test` runs CPU-only.
- **Frontend:** vitest 4 + `@vue/test-utils` + `@testing-library/vue`, `happy-dom`. Priority coverage: capability-matrix-driven form logic (ported `generateCapabilities` + `GenerateForm` enable/disable per family), SSE reducer (progress event stream → job card states, including dropped-stream reconciliation via `/api/queue`), chain composer stage editing, updater startup/manual-check/install/error/recovery states, and api client (mocked `fetch`, header injection, error envelope parsing). Tauri IPC mocked with `@tauri-apps/api/mocks`.
...
- **CI:** `.github/workflows/desktop.yml` runs desktop frontend formatting/type/tests, updater publishing-script tests (including tamper/wrong-key rejection and retention selection), and the macOS Rust fmt/clippy/test/bundle gates for desktop-relevant pushes and pull requests. A desktop-relevant `main` push that passes both jobs continues through the signed/notarized Nightly distribution and public-manifest verification; tagged Stable artifacts remain in `release.yml`. The desktop cargo root remains excluded from the workspace CI.
...
- **M6 — Polish & ship-readiness:** settings surface (config.toml + DB settings via existing endpoints/expand config), notifications, window-state, single-instance, drag-drop images from Finder, keyboard shortcuts, empty/error states, signed Stable/Nightly updater with persistent rollback supervisor, `mold-desktop` Nix package + signing flow + verify-bundle, docs page.
