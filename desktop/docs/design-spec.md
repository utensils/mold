...
**Light variant policy ("Lights on"):** chrome inverts to warm paper — Bath→`#F2EBDE`, Bench→`#FBF6EC`, Rebate→`#231D18`; semantic colors darken for AA contrast. Actual generated media remains on a color-stable print surface, while an empty generation canvas follows the light chrome so it cannot be mistaken for a failed black preview. Follows system appearance by default; overridable in Settings.

**Theme families:** **Mold** is the desktop fresh-install default and translates the website/logo identity into product semantics without using gradients as UI decoration: cyan is Halide for queued, latent, and reference states; magenta is Safelight for active, selected, and focus states. Its dark surfaces are violet-black and its light surfaces are neutral lavender-white. The iPhone fresh-install default is **Safelight**, the original darkroom palette; existing saved choices remain unchanged. Users choose a family and System/Dark/Light appearance independently; new users start on System. Every combination maintains WCAG AA text contrast and 3:1 interactive boundaries; generated media continues to use the fixed print surface in both families.
...
- **Command palette (⌘K):** navigation, actions ("Pull flux-dev:q8", "Cancel all queued", "Switch to remote engine"), model search, prompt-history search, and gallery search (matching prints by prompt / model / filename, deep-linking to the print's lightbox via `/gallery?print=`) in one field. Elevation-2, the one blurred surface besides the sidebar.
- **Movement model:** sidebar click or ⌘1–⌘5; deep links everywhere (gallery item → "Reuse settings" → Generate prefilled; queue chip → owning screen; chain stage → Chains editor). Back/forward via ⌘[ / ⌘]. Reuse settings also restores the print's **input image** when its metadata carries the additive provenance keys (`source_image_name` / `source_image_sha256`): the local content-addressed source stash is read first (submit-time copies of the exact bytes, bounded), then a cross-host gallery filename match from the print's origin host; a keyed print whose source is gone toasts, keyless old prints skip silently.
...
- **Prompt composer:** multiline, Schibsted 15px, grows to 6 lines. `⌘↩` runs the current primary action. With Batch 1, **Expand** (`POST /api/expand`) performs a one-prompt rewrite and offers restore-to-original undo. With Batch N greater than 1, it becomes **Prepare N variations**: Mold resolves the Generate host once, validates exactly N non-empty results, and opens a responsive inline review workspace with numbered editors, host provenance, regenerate-all, intentional removal, discard, and **Generate N variations**. Upstream source/model/host/count changes preserve the workspace with named stale reasons and block submission. A missing expansion model uses one shared compact inline surface in either mode: Connecting → Starting → Queued → Pulling (percentage, bytes, file, ETA) → Ready with explicit expansion retry, plus same-host failed/cancelled recovery. The surface consumes the existing exact-host download stream and never redirects to Catalog. Negative prompt is a collapsible second field shown _only_ for CFG families (sd15/sdxl/sd3/wuerstchen), capability-gated rather than merely disabled.
- **iPhone prepared composer:** the same lifecycle is recast as a single-column Safelight workspace with 44pt actions and 16px editors. The selected remote host's ID, endpoint, Keychain credential, and server instance are frozen through preparation, pulls, source preprocessing, and Develop. Editors lock while replacement/submission work is pending, while Discard remains available to invalidate it; delayed completion restores focus only if the replaced control still owned focus.
- **Model picker:** popover with installed models grouped by family; each row shows a **source mark** (HF tile / Civitai diamond / local disk — neutral currentColor monograms, `SourceGlyph.vue`), the complete model name (wrapping rather than ellipsizing), multi-host availability on a secondary line when present, and a residency dot (Safelight = on GPU; parked/cold states and quant-tag chips remain aspirational). The selected-model control also wraps long names and grows vertically. Footer: **"Browse all models →"** deep-links to `/models` (`?type=video` for video families), where one searchable, unified Catalog list merges the live catalog with the all-host Installed union — installed rows deduped by name, host-badged, sorted first — behind All / HuggingFace / Civitai / Installed source chips (All default), with host-routed actions, All / Images / Video media chips, NSFW tag chips, and primary-weights vs shared-runtime size labels. Active host-scoped pulls appear both there and on the matching host detail page.
...
- **img2img / mask / control:** a drop well under the composer's `img2img ▾` split-button. Dragging any image over the window dims the app and shows three labeled drop zones — **Source** / **Mask** / **Control** (Control zone only for sd15) — with paper-rebate outlines. Dropped source sets a strength slider (0.75 default) and previews as a small print clipped to the corner of the canvas. Qwen-edit mode swaps the well for an ordered multi-image tray ("First image is the edit target"). LTX-2 gets the same single well (image-to-video: the source conditions frame 0 at the chosen strength) with no mask surface and no Pad repaint fit option — a mask-dependent policy coerces to centered Crop fill; plain ltx-video renders no well (its engine has no img2vid path).
...
- **Generation lifecycle:** click Generate → job chip appears in the queue strip (cold grain, `Queued #2`) → `/api/generate/stream` events drive the Develop in the canvas → complete → print fixes in, edge code stamps, result auto-saved to gallery, batch dots (◉◎◎◎) fill in as `base_seed+i` siblings finish. A prepared batch sends each edited prompt through its one frozen host route and retains the source prompt in `original_prompt`; every sibling also carries the durable `batch_id` and one-based `batch_index`/`batch_count` shown in Gallery. Partial failures identify the affected variation while keeping successful prints. Zombie reconciliation: `/api/queue` polled to dead-letter chips whose SSE dropped, shown as Stop-frozen grain with a Retry action.
...
- **One print, wherever it lives:** the All view collapses cross-host copies of the same print into one tile — by filename (auto-saved remote results keep the origin server's gallery filename via the complete event's additive `filename` field) and, for legacy copies that minted their own names, by seed + byte-size identity. The tile prefers the local copy for media and actions and lists every location; host chips still show each host's raw bucket. Right-click **Save locally** stays visible everywhere and grays out once a local copy exists (under any name); saves carry the origin row's metadata so video copies keep their true dimensions.
...
- **Queue vs residency, disambiguated:** queue depth/capacity, a Stop-toned `PAUSED` marker, and a `Jobs →` link live in the QUEUE header. Below it, the host's whole server queue renders as compact read-only rows sharing the Jobs view's vocabulary — temperature dot (Safelight running / Halide queued), `RUNNING · GPU 0` / `QUEUED #2` code, model, `OTHER CLIENT` tag, elapsed wall-clock — via a page-scoped 5 s poll of the jobs store (`refreshHost`); management (cancel, lanes, pause) stays in Jobs, but clicking a row opens a right-side job drawer (state, host, owner, elapsed, and the submitted settings from the queue listing's additive `metadata` field) whose **Load settings in Generate** prefills the composer; the action grays out against servers that don't share settings. Models resident in the engine render under an explicit `LOADED` label with a Safelight residency dot — they must never read as queue entries — and each chip carries an unload control that frees that host's GPU.
...
**Menu bar:** **mold** (About, Check for Updates…, Settings… ⌘,, Quit) · **File** (New Generation ⌘N, New Chain, Open Chain Script…, Import Image for img2img…, Export Selection…, Reveal in Finder) · **Edit** (standard + Copy Seed, Copy Prompt) · **Generate** (Generate ⌘↩, Expand Prompt ⌘E, Randomize Seed ⌘R, Duplicate Last ⌘D, Cancel Job ⌘.) · **View** (screens ⌘1–5, Toggle Sidebar ⌘\, Appearance ▸, Actual Size/Zoom, Developer Tools ⌥⌘I) · **Window** / **Help** (Mold Documentation → utensils.io/mold, API Reference → /api/docs, Open Logs). The webview's own right-click menu (Back / Reload / Inspect Element) never appears anywhere — only genuine text-editing fields keep the native menu (spellcheck/paste; `allowsNativeContextMenu`), everything else opens the app's own context menu or nothing, and **View → Developer Tools** is the sanctioned devtools entry point (the `devtools` cargo feature keeps it working in release builds).
...
- **Notifications:** "Generated — lighthouse at dusk" with thumbnail on job complete _when the app is backgrounded_; "Chain finished · 243 frames" ; "Pull complete — flux-dev:q8". Bundled macOS builds send through Apple's UserNotifications framework so Notification Center derives Mold's icon from the signed bundle identity; Tauri's notification plugin remains the cross-platform fallback. Clicking focuses the relevant item.
