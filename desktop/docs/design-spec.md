...
**Theme families:** **Mold** is the fresh-install default and translates the website/logo identity into product semantics without using gradients as UI decoration: cyan is Halide for queued, latent, and reference states; magenta is Safelight for active, selected, and focus states. Its dark surfaces are violet-black and its light surfaces are neutral lavender-white. Safelight remains available as the original darkroom palette, and existing settings written before theme families migrate to Safelight instead of changing unexpectedly. Users choose a family and System/Dark/Light appearance independently; new users start on System. Every combination maintains WCAG AA text contrast and 3:1 interactive boundaries; generated media continues to use the fixed print surface in both families.
