...
- **Model picker:** popover with installed models grouped by family; each row shows a **source mark** (HF tile / Civitai diamond / local disk — neutral currentColor monograms, `SourceGlyph.vue`), name, and residency dot (Safelight = on GPU; parked/cold states and quant-tag chips remain aspirational). Footer: **"Browse all models →"** deep-links to `/models?tab=catalog`, where installed entries group at the top under an Installed divider. Selecting a non-installed catalog model queues a pull and shows "Pulls on first generate" (aspirational).
