...
**Live catalog search** (`GET /api/catalog/search`, `useCatalog.ts`): HF + Civitai proxy, 5-min in-proc cache. Query params: `q`, `family`, `kind`, `source` (hf/civitai), `include_nsfw`, `page`, `page_size`. Catalog IDs `cv:<id>` / `hf:<author>/<name>` usable directly as model names (auto-resolve + download). Endpoints: `/api/catalog/families`, `/api/catalog/installed`, `/api/catalog/*id` (GET entry / POST dispatch = download). Civitai LoRAs carry `trained_words` (trigger phrases). `MOLD_CATALOG_DISABLE=1` disables. LoRA list: `GET /api/loras?model=` filters by compatible family.
