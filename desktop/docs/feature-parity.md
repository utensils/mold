...
- **downloads drawer** (web/desktop): `GET/POST /api/downloads`, `DELETE /api/downloads/:id`, live SSE stream (cancelable, progress bytes/percent). Desktop keeps one stream per selected download host, pins active rows at the top of the Catalog view, and labels/routes each row by origin (source glyph + target host; primary rows resolve to the local host's label). Each row carries a status chip (active/queued/failed/cancelled/completed), and active rows show the current file, files done/total, and a live ETA computed client-side from a 10 s sliding rate window (`etaByJob` in `stores/downloads.ts` — stalls honestly show no ETA). Settled jobs land in a collapsed **History** section instead of vanishing; failed entries keep their error message and offer **Retry**, which re-starts the download on the same host it failed on (catalog ids re-enter the catalog dispatcher, plain names re-POST `/api/downloads`).
