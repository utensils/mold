...
- **Cloud provisioning**: the desktop RunPod workspace covers API setup, account/spend status, GPU stock, datacenters, network volumes, pod create/start/stop/delete/logs, automatic refresh, and connecting the app to a pod. The CLI additionally provides doctor/get/usage and `runpod run` one-shot generation on fresh/warm pods. Lambda provisioning remains CLI-only (doctor/availability/deploy tunneled web UI/status/logs/tunnel/ssh/filesystems/terminate/reset).
