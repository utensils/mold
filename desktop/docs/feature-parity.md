...
- Desktop (`ExpandControl.vue`, `PreparedExpansionBatch.vue`, `ExpansionPullStatus.vue`): Batch 1 keeps one-shot ⌘E in-place rewrite with restore-to-original undo and freezes its resolved route through Generate. Batch N greater than 1 resolves one concrete Create host, requires exactly N non-empty results, and opens an inline editor before queuing. Removal updates Batch; reducing two prompts to one requires an explicit inline confirmation. Regenerate-all replaces the set only after success, while source/model/host/count changes preserve it with named stale reasons. Every sibling uses the stored route, its edited prompt, the shared source prompt as `original_prompt`, and additive `batch_id`/`batch_index`/`batch_count` provenance. Missing expansion-model recovery never falls back or loses the ordered prompts; it reuses the exact host's download store to show Connecting, Starting, Queued, detailed Pulling progress, Ready, and terminal retry inline.
