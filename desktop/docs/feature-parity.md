...
- **Cloud provisioning**: `mold runpod` (doctor/gpus/datacenters/list/get/create/stop/start/delete/connect/logs/usage + `runpod run` one-shot generate on fresh/warm pod) and `mold lambda` (doctor/availability/deploy tunneled web UI/status/logs/tunnel/ssh/filesystems/terminate/reset). Both configurable via `config set runpod.api_key`/`lambda.api_key`.
