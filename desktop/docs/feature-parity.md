...
| `batch_id`                           | string?                        | —                         | Additive durable identity shared by prepared desktop siblings.                                                                                                                                                                                                                             |
| `batch_index`, `batch_count`         | u32?                           | —                         | Additive one-based prepared sibling position and total; retained in output metadata.                                                                                                                                                                                                       |
...
- **download monitoring (web/native apps)**: `GET/POST /api/downloads`, `DELETE /api/downloads/:id`, and the live SSE stream expose cancelable progress. Desktop keeps a stream for every ready host, pins rows in Catalog, mirrors them in host detail, shows status/current file/files done/total/live ETA, and retains settled History with same-host Retry. iPhone keeps the same multi-host stream ownership within Catalog and host detail, with immediate Pull button status and cancellation. The web drawer remains scoped to its serving host.
...
| Navigation  | Generate, Gallery, Catalog, Hosts; header-pushed Settings                                                                                     |
| Hosts       | Bonjour `_mold._tcp`, IP/DNS/HTTPS, Tailscale MagicDNS, Keychain API keys, detailed telemetry/models/downloads/queue                          |
| Generate    | Shared capability/request logic, proportional resolution controls, Random/Fixed seed, independent batch queue, current image/video parameters |
| Gallery     | All saved hosts merged newest-first, full-size image/video, Range seeking, horizontal navigation, explicit prompt/source reuse                |
| Catalog     | Installed/live union, browse-host/target-host separation, model detail/actions, immediate Connecting/Starting/Queued/Pulling status           |
| Settings    | Mold/Safelight × System/Dark/Light, host shortcut, version, remote-only policy, and TestFlight update channel                                 |
| Interaction | Safe areas, 44pt controls, 16px editable text, no WebView document zoom, no shell rubber band, scoped gallery swipe                           |
