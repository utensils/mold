...
/// output file (embedded `mold:parameters` metadata included), so the local
/// gallery reads full provenance back out of the file. Returns the saved
/// filename.
#[tauri::command]
pub async fn save_output_bytes(filename: String, data_b64: String) -> Result<String, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if let (Some(db), Some(format)) = (
            ...
            let timestamp = std::time::SystemTime::now()
                ...
                .unwrap_or_default()
                .as_secs();
            let (metadata, _synthetic) =
                mold_db::metadata_io::read_or_synthesize(&path, format, &saved_name, timestamp);
